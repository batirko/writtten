...
- [x] `src/model/registry.ts`: `PROVIDERS`, `PROVIDER_IDS`, `resolveProvider(id)`, `catalogFor`/`defaultModels`, and `withSelection` (single-model paid routing for `paidFast`; the `paidStrong` **fallback tail** exception is noted below). `factory.createRouterForSelection(selection, apiKey, paidKey)` is the new entry point: Gemini reuses the existing `createRouter` path (rotation pools + the mock/record `wrap`); paid providers drive the generic engine with one selected model per tier. `createRouter` is unchanged, so the evaluator call sites and the `../model/gemini` test mocks keep working (the mocks now spread `importOriginal` so the added `geminiAdapter` export survives).
  > **"Paid providers don't rotate" now has one deliberate exception (2026-07-14, `strong_tier_eval_reliability.md`).** `withSelection` keeps `paidFast` single-model but routes `paidStrong` as `[selected, ...rest-of-strong-catalog]` — a **failure-only** fallback so a strong-tier timeout on a heavy contradiction sweep rotates to the next strong model instead of dropping the whole call. It's still "no rotation" in the normal case (the selected model answers first every time); the tail engages only on a retryable failure. Single-strong providers (Anthropic) collapse to one entry, so nothing changes for them. _(2026-07-27: Anthropic's strong catalog grew to `["claude-sonnet-5","claude-opus-5","claude-opus-4-8"]`, so it carries a real tail too — no provider is single-strong any more.)_
...
- [x] **"Know how to get the key" — kill the three moments of doubt:**
  ...
  - _Did it work?:_ a per-provider **"Ping model"** button (`src/model/ping.ts` → `pingProvider`, `data-testid="ping-model"` / `ping-verdict`) that fetches once and **decodes the outcome into plain language** — "Invalid key" / "Valid key — billing not enabled" / "rate-limited" / "Couldn't reach the provider — check your network or CORS". Verified in-browser end to end: a bad `sk-ant-…` key → 401 → "Invalid key." (also proves Anthropic's browser-access CORS path works). **The button was removed 2026-07-25** (settings rework) — don't go looking for it. The question it answers and the decode it produced are unchanged; a debounced auto-verify now fires on every key change and `keyStatusView` prints the outcome on the field's own subtitle, per field. `pingProvider` is what that auto-verify calls, so `ping.ts` is untouched.
...
>
> **Updated 2026-07-27:** thinking is disabled on **every** Anthropic model now, not just Sonnet — the rule keyed off `model.includes("sonnet")`, so `claude-opus-5` quietly ran adaptive thinking (billable, and sharing the `max_tokens` budget with the answer). The exception is `claude-fable-5`, which 400s on an explicit disable and gets the field omitted instead. None of this is a hardcoded id check any more — see `docs/mechanics/model-request-negotiation.md`. Strong catalog grew to `["claude-sonnet-5","claude-opus-5","claude-opus-4-8"]`; `claude-sonnet-5` is still the routed default.
