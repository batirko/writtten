...
- **Decided once at the App boundary** (`App.tsx`), from the key configuration: an env `VITE_GEMINI_PAID_KEY`, or the Gemini key's tier **auto-detected** (`ping.ts → detectGeminiTier` probes `gemini-2.5-pro`; persisted as `writtten_key_tier`). A strong tier also promotes the key into the `paidKey` routing slot so `strong()` reaches the paid pool. (Paid providers — OpenAI/Anthropic — are always strong capability.) The manual "capable model" checkbox was removed 2026-07-07 in favor of auto-detection.
