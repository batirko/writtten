...
| ---- | ------------- | -------------------------------- |
| A `contradiction` | timeline · intra-section · sweep-fidelity · **`contradiction-sla-family`** (OBS-038 triplet, landed 2026-07-17 — free-tier recorded) · **`contradiction-sweep-paraphrase`** (OBS-031 message fidelity, landed 2026-07-18 — free-tier recorded) | `contradiction-short-paste` (UX-016) |
...
**5. The consolidated live-recording session (the "merged decide-items" plan milestone — decisions made 2026-07-16: record, both).** **✅ Recorded 2026-07-18** — all five fixtures captured at the free tier (`gemini-3.1-flash-lite`) and wired into the Tier-1 corpus (21→26). The OBS-027 discrimination held at the weak free tier (the "weak model may ignore the Established-elsewhere block" risk did not materialize). The recorder (`record.test.ts`) gained a `sweep` branch to capture `contradiction-sweep-paraphrase` (it previously handled only per-section fixtures). No product-code change. See the `docs/plan.md` milestone record for the per-fixture outcomes. The OBS-027 prompt-assembly guard proves the *input* reaches the model; V1 proved behavior is where precision lives — so both deferred "decide: record or accept the existing guard" items resolve to **record**:
