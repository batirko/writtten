...
1. Copy the following Markdown text:
   ...
   ```
1. Paste it into the TipTap editor.
2. Open the dev console and run `await window.__sidecar__.getState()`.
3. **Assert:** The blocks array contains 4 distinct blocks: two headings and two paragraphs. It should NOT be a single large paragraph block.
...
1. Create a `test.md` file locally containing:
   ...
   Imported text body.
   ```
1. Click the "Import Document" button in the sidecar header.
2. Select `test.md`.
3. **Assert:** The editor clears its old content and replaces it with the contents of `test.md`. The `# Title` should render as a top-level heading.
