...
- No external-agent collaboration ("agent experience"). We will not expose a doc-as-shared-workspace bridge (presence/state/event-stream/edit APIs) for outside agents to co-author in, as competitors like Proof (proofeditor.ai) do — that is the AI-as-author model we invert. See `docs/snapshots/2026-06-13_competitor_proof_editor.md`.
  **Amended 2026-07-19 (owner):** this non-goal forbids agents as _co-authors_; it does not forbid agents as _observation sources_. A read-only, taxonomy- and register-enforced boundary through which the user's own coding-agent session submits **observations** — never edits, suggestions, comments, or presence; the protocol contains no message that can carry a document mutation — is in scope and scheduled (`docs/projects/agent_connected_eval.md`, the bring-your-own-agent eval source). The distinction: an observation source provokes the author's thinking exactly as the built-in evaluator does, through the same fixed taxonomy and register discipline; a co-author replaces it. Everything the first sentence refuses stays refused.
