 ...
 *   - every top-level bullet in the "Phase 6" and "Discovered / unscheduled"
 *     sections (those backlog sections use bullets, not checkboxes).
...
function collectItems(): Item[] {
  ...
  lines.forEach((line, i) => {
    ...
    // Non-checkbox top-level bullets are milestones only in the bullet-style
    // backlog sections (Phase 6 and Discovered / unscheduled).
    const isBacklog = section.startsWith("Phase 6") || section.startsWith("Discovered");
