...
- [x] **Make diagnostics survive a remount.** Done — the `llmLogger` now mirrors its raw entries into `sessionStorage` (per-tab, survives a reload; DEV-only, since the panel is DEV-gated and entries carry prompt text), debounced, revived on construct; `clearLogs()` wipes the mirror. The debug switch (`debugMode` in `ControlCenter`) is persisted to `localStorage`. Only the entries are mirrored — the derived session/quota counters are live-session aggregates and intentionally reset (the per-call rows are the diagnostic value). Guarded by `logger.persistence.test.ts`.
- [x] **Add a top-level ErrorBoundary.** Done — `src/ErrorBoundary.tsx` (a class boundary) wraps `<App/>` in `main.tsx` (genuinely top-level, so it also catches App's own render); a render/lifecycle crash shows a calm recovery surface (document is safe on-device + Reload) instead of a white screen. Note: React error boundaries don't catch *async* errors (a rejected provider promise) — this covers the render path only, the different remount path from the HMR one above. Guarded by `ErrorBoundary.test.tsx`.
