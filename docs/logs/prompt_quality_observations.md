...
- [ ] **Rhetoric not extracted as `fact_claim`** — under relaxed document classes, stop promoting hyperbolic/rhetorical emphasis and narrative framing to ledger `fact_claim`s; this is the upstream source of the false `unsupported_claim` + `strategic_tension` cascade on essays. Composes with `document_type_calibration.md` (extraction-side calibration). (See OBS-037.) **Design settled 2026-07-16** → `document_type_calibration.md` § _Extraction & tension calibration for rhetoric_ (extraction rule rides the class-gated calibration blocks incl. `unknown`; a second lever adds the author-reconciled-tension rule to the contradiction prompts' NOT-A-CONFLICT bucket — the FP class V1 Run 1 measured on real PRDs).
