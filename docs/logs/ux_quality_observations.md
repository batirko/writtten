...
**Date:** 2026-07-21\
**Status:** open — **design settled 2026-07-21** (owner design session), not built. Three decisions, one per surface: the welcome modal folds to "one claim, one ask"; the connect panel's permission block moves out entirely to the pre-flight milestone (the largest single trim, and the reason no aggressive fold is needed here); the prompt becomes a peek that unfolds on click. Full record on the Phase 8 milestone — including why a peek-with-unfold is **not** the fade UX-032 rejected.\
