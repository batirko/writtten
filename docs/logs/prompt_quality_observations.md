...
- [ ] **Doc-level prompt: close the index/`§N` leak (parity with UX-017)** — mirror the "never name a claim by its `[N]`/bookkeeping index, quote/restate instead" instruction from the four contradiction prompts into `DOC_LEVEL_SYSTEM_PROMPT`, and forbid `§N` section addressing unless the document actually has numbered sections. Extend the `evalRatchet.test.ts` label-leak lint to cover doc-level `text` outputs so the class can't reopen on a third surface. Prompt-only. (See OBS-034.)
- [ ] **Doc-level Block Summaries must be document-ordered** — the doc-level prompt feeds summaries/claims in **alphabetical** order (`evaluator.ts:703–705,710`), reusing the request-hash sort as the model's positional `[1]/[2]` input, so `structure_flow`/ordering observations reason over a scrambled sequence. Fix: keep the alphabetical order for `docStateHash`, but build the `Block Summaries`/`Claim Ledger` prompt sections from a separately document-ordered list. (See OBS-035.)
