...
**Fix (2026-07-21):** `src/services/agentOffer.ts` — one predicate, `agentPathOffered()` = preview flag AND `currentAgentBrowserSupport().supported`, consumed by all three on-ramps so they cannot drift apart again (the `engineReadiness.ts` precedent from UX-045, same problem shape). The welcome modal and the keyless banner drop the CTA, and the banner's sub-line drops the agent clause with it — one flag gates the link and the prose, since a promise in the copy is the same offer as a button. The Engine control renders **no agent tab at all** on WebKit, per the amendment; the segmented control is dropped entirely rather than left showing one option, matching the flag-off case, and in its place the group carries a line naming the limitation with a link to `/agent/`.

**Where the information panel landed** — the amendment's one open question. It was already built: `/agent/` is a static page that explains the feature end to end and already has an honest Safari section, and Settings already linked it. So nothing new was written; the line under the Engine label routes there. A disabled-with-reason tab was rejected on the entry's own rule (a tab is an offer) and on touch: iOS is all WebKit, so most affected users can't hover, and a disabled control gives no reason on tap.
...
**Verified in real Safari** (2026-07-21, production build over https with `?agent=1`): `navigator.vendor` Apple, `location.protocol` https, flag on — no agent tab, no CTA on either on-ramp, no `/agent/i` match anywhere in the keyless banner, and `localStorage["writtten_engine"]` **never written**, including after dispatching `writtten:open-settings` with the `connect-agent` intent directly (Settings opens; the slot does not move and no pairing starts). Re-checked at 375 px: no horizontal overflow, note wraps. Tests: `src/sidecar/agentOffer.webkit.test.tsx` mocks the support module unsupported and covers all three surfaces plus the `isBuiltinEngineActive()` invariant.
