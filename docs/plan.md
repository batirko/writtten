...
| [visual_style](projects/visual_style.md)                                 | idea   | 5                                                                | The visual design system — type, colour (OKLCH tokens), spacing, elevation, motion, per-component language. Calm/editorial: serif writing canvas, sans feed, ink-indigo brand accent on warm paper. Light-first; dark deferred.                                                       |
| [ui_interaction_mechanics](projects/ui_interaction_mechanics.md)         | idea   | 5                                                                | Canonical interaction contracts for feed↔editor — hover/focus highlight, click-scrolls-to-span, dismiss + Undo toast (with suppression rollback), card anatomy, the two "also noticed" drawers. Makes partly-built Phase-4 mechanics intentional; scopes against R7b/R3c.             |
| [onboarding_first_run](projects/onboarding_first_run.md)                 | idea   | 5                                                                | First-run: a one-time dismissible welcome moment framing the inversion + an optional one-click "See it in action" example (planted contradiction) so the hero is witnessable immediately, the quiet-by-design empty states, and no upfront setup/key gate.                            |
| [editor_formatting_ux](projects/editor_formatting_ux.md)                 | idea   | 5                                                                | Discoverable formatting for the writing canvas — a selection bubble menu (inline marks + links) + a slash menu (block types), appear-on-demand with zero standing chrome, + a Link extension. Closes UX-004: formatting works but is invisible.                                       |
| [doc_level_anchoring](projects/doc_level_anchoring.md)                   | idea   | 5                                                                | Give doc-level checks (structure_flow, underexposed_topic, audience_mismatch) the ability to anchor — section-level via blockId + optional substring fallback — and tighten prompts so categories stop bleeding. Resolves R4 / OBS-015/016/018, UX-001/009.                           |
...
| [canvas_content_types](projects/canvas_content_types.md)                 | idea   | 5 · 6                                                            | Decide & bound which rich-content types the canvas accepts on paste / lets the user create (tables, images, links, task lists) and how each interacts with the section/eval model. Today StarterKit-only → pasting a table or image silently loses content (a trust bug).             |
...
- [ ] **Canvas content types (paste-in & create)** — decide and bound which rich-content types the canvas accepts on paste and lets the user create (tables, links, images, task lists), and how each interacts with the section/eval model + Markdown round-trip. **Phase 5 floor: stop silent content loss** — today StarterKit-only means a pasted table/image vanishes; at minimum degrade unsupported paste to a visible/editable form. Tables are the highest-value PM-persona add. Distinct from _Editor formatting UX_ (that's the control surface; this is the schema). → see `docs/projects/canvas_content_types.md` — 🟠 Med · 🧠
