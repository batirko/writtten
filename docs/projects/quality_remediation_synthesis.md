...
One schema change — let doc-level checks optionally return anchoring targets — plus prompt tightening fixes OBS-015, OBS-016, OBS-018, UX-001, and UX-009 together. The already-shipped `strategic_tension` split (**OBS-004**, done 2026-06-04) is the precedent: give the model the right bucket and the misclassification stops. It graduated into its own build-ready spec on 2026-06-17 (section-level anchoring (blockId) + optional substring fallback), but was **dropped 2026-07-05 (won't do)** — a section-anchoring + hover-highlight slice was prototyped and reviewed, and the product owner decided the feature didn't earn its place; the spec file was removed. The underlying issues (OBS-015/016/018, UX-001/009) are consequently **not** scheduled: the doc-level cards stay whole-document (no anchor/highlight), and the `audience_mismatch`/`structure_flow` category bleed remains a known, unaddressed signal-quality wrinkle.
