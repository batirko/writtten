...
**Materiality floor (added 2026-07-20).** We push on every settle so `GET /doc` stays a complete, current snapshot — but `docVersion` is the *separate* question of whether the agent should be **woken to re-review**, and it bumps only on a material change. Two reasons it may not bump:
...
- **Non-material edit** — the words are unchanged and only their partition moved. `docVersion` was previously gated on a byte-exact hash over `[title, stage, sections]`, which answers *did bytes change* when the question is *could the conclusions change*. Splitting a heading into its own section changes `sections[]`, so it woke the agent for a full re-review; in the field session that agent reported back "No new content — just the heading was split into its own section," at a measured **~4.1k tokens** out of the user's own agent budget. Watch mode multiplies this.
