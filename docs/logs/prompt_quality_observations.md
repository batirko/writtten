...
- [ ] **Jargon allow-list** — foundational domain vocabulary should not be flagged as undefined. Two preset layers: a general PM/product-process preset on by default, plus per-sub-domain presets and a user dictionary. Tie into the jargon allow-list Phase 4 milestone. (See OBS-003, OBS-005.) **Direction superseded 2026-07-16 (owner):** after V1 Run 1 quantified the noise at 21–53 flags per real PRD, the fix is **inferred-audience calibration + once-per-term dedup + a section cap**, explicitly _not_ bigger preset lists — design in `document_type_calibration.md` § _Audience-relative jargon calibration_; the shipped user glossary/allow-list machinery stays.
...
- [x] **Scope-excluded claim tagging** — tag claims extracted from "Out of scope" / "Non-goals" sections (e.g. `scope: excluded`) at extraction time, and have the contradiction prompts skip or heavily downweight conflicts where either side is `scope: excluded`. Extends OBS-027's heading-governs-intent fix beyond the span-check prompt into claim extraction and the contradiction prompts. (See OBS-030.) **Done 2026-07-18** — shipped deterministically/code-side (heading-derived `scope: "excluded"` tag on `ClaimLedgerEntry` + candidate-pool filter in both contradiction paths; no prompt edits ⇒ zero fixture re-key). Owner chose code-side over the prompt-emitted variant. See OBS-030 Resolved note.
...
- [x] **Doc-level prompt: close the index/`§N` leak (parity with UX-017)** — **shipped 2026-07-12 (#178).** Mirrored the anti-index rule into `DOC_LEVEL_SYSTEM_PROMPT` (never name a summary/claim by its `[N]` index; never invent `§N` unless the doc uses numbered sections — refer by quote/heading/subject). Extended the `registerLint` label-leak guard to the doc-level observation types: broadened `CLAIM_INDEX_RE` to catch the `claim [3]` / `claims [1] and [2]` / `block [2]` bracket forms and added a `section-number` rule (`§\d+`), both scoped to `missing_topic`/`underexposed_topic`/`audience_mismatch`/`structure_flow`; unit-covered in `registerLint.test.ts`. Prompt-only (re-keyed the example doc-scan recording). (See OBS-034.)
- [x] **Doc-level Block Summaries must be document-ordered** — **shipped 2026-07-12 (#178).** `evaluateDocument` now builds the `Block Summaries` / `Claim Ledger` prompt sections from a **document-ordered** view (reads persisted `DocumentRecord.content` via `loadDocument`, ranks summaries/claims by their block's reading-order position; stable secondary = alphabetical), while the `docStateHash` keeps the alphabetical order for replay determinism. Absent blocks degrade gracefully to the tail. Verified by a new `evaluator.test.ts` ordering test + live keyless-demo replay. Note: the demo's doc isn't persisted (`setContent(emitUpdate:false)`) so it uses the alphabetical fallback (harmless — no `structure_flow` card); real typing sessions persist and get reading order. (See OBS-035.)
- [x] **Rhetoric not extracted as `fact_claim`** — under relaxed document classes, stop promoting hyperbolic/rhetorical emphasis and narrative framing to ledger `fact_claim`s; this is the upstream source of the false `unsupported_claim` + `strategic_tension` cascade on essays. Composes with `document_type_calibration.md` (extraction-side calibration). (See OBS-037.) **Done 2026-07-18** — both levers shipped (extraction rule in `sectionCalibrationBlock` for relaxed + `unknown`; author-reconciled-tension rule in the four contradiction prompts). Corpus + demo re-keyed byte-identically offline; two weak-tier live fixtures (`rhetoric-extraction`, `reconciled-tension`) confirm the behaviour. See OBS-037 Resolved note.
- [x] **Carve-out covers rhetorical-question apprehension** — extend the fast-tier first-person carve-out so interrogative/hypothetical framings of a personal concern ("won't my writing skill atrophy?") are caught alongside the declarative "I fear…/I'm concerned…" forms it already lists. Tactical, feeds `document_type_calibration.md`. (See OBS-036.) **Done 2026-07-09 (#162):** the framing-invariance sentence shipped in `MERGED_SYSTEM_PROMPT` (declarative / rhetorical-question / hypothetical all covered), guarded by the `rhetorical-apprehension` ratchet fixture (recorded live at free tier). _(Noticed 2026-07-16: this box was left unticked when #162 shipped — the `docs/plan.md` Phase-8 OBS-036/037 milestone accordingly narrows to the rhetoric-extraction half above.)_
...
**Date:** 2026-07-05\
**Resolved:** 2026-07-18 — shipped as **deterministic, code-side** tagging (owner decision over the model-emitted-scope alternative, which would be unreliable on the weak free tier and force a full live re-record of the corpus). A new optional `scope: "excluded"` field on `ClaimLedgerEntry` (DB v9→v10, additive/no-backfill) is set at extraction time from the governing section heading (`isExcludedScopeHeading` in `src/services/scopeHeading.ts`, keyed on `members[].isHeading` set by `resolveSections`), and **both** contradiction paths — the per-section check and the bulk-paste sweep in `src/services/evaluator.ts` — filter excluded claims out of the candidate pools (the "skip" option; done in code, not via a prompt rule). Re-keys **zero** fixtures: the corpus harness builds members without `isHeading`, so the feature is inert there and every request hash stays byte-identical (full ratchet + `exampleReplay.sync` green). Guarded by `src/services/scopeHeading.test.ts` + OBS-030 discrimination cases in `evaluator.test.ts`.\
...
### OBS-032 — `unsupported_claim` card body just restates the cited sentence verbatim

**Date:** 2026-07-08\
**Resolved:** 2026-07-08 — the `unsupported_claim_observations` spec in `MERGED_SYSTEM_PROMPT` now carries the same "don't restate, name the gap" instruction the `clarity_observations` spec already had, with a worked example (`"No data or trend cited for the decline-rate claim."` — never `"Our decline rate is climbing."`). `src/services/evaluatorPrompts.ts`. The 17 Tier-1 ratchet fixtures were re-keyed offline (byte-identical responses; only the section-eval prompt hash shifted — same maintenance as #44).\
**Prompt tier:** fast (section-eval, `MERGED_SYSTEM_PROMPT`, `src/services/evaluatorPrompts.ts:52`)\
**Type flag:** unsupported_claim (Medium confidence)\
**Input excerpt:** Background section: _"Our transaction decline rate has been climbing for three consecutive quarters…"_ Observed on `gpt-5.4-mini [paid]`. Returned `unsupported_claim_observations[].text` = _"Our transaction decline rate has been climbing for three consecutive quarters."_ — i.e. **identical to the cited `substring`**.\
**Expected:** The card body (`text`) should name what evidence is missing — an insight the author acts on — while the italic quote above it (`anchorText`/`substring`) already shows the cited span. The two should not be duplicates.\
...
### OBS-034 — doc-level prompt leaks evaluator-internal claim indices (`claim [3]`) and invents `§N` section numbers into user-facing copy

**Date:** 2026-07-08\
**Prompt tier:** doc-level (strong, gemini-2.5-pro, `DOC_LEVEL_SYSTEM_PROMPT`, `src/services/evaluatorPrompts.ts:59`)\
**Type flag:** all doc-level (missing_topic, underexposed_topic, audience_mismatch, structure_flow)\
**Input excerpt:** A personal essay on AI and PM writing, no headings/section numbers. Doc-quality returned, verbatim: _"The product's core functionality, defined in claim [3] as highlighting \"key issues\", is not detailed enough…"_ (underexposed_topic); _"The informal phrasing in claims [1] and [2] may not align…"_ (audience_mismatch); _"…introduces the solution in §1 before establishing the problem it solves in §2."_ (structure_flow).\
**Expected:** User-facing copy that refers to the author's own text by quote/paraphrase, never by the evaluator's ledger index (`claim [3]`) and never by a section number the document doesn't have (`§1`).\
**Actual:** Two internal-addressing leaks in one call. (1) The doc-level request body feeds an indexed `Claim Ledger:\n[1] (kind): …` and `Block Summaries:\n[1] … [2] …` (`src/services/evaluator.ts:742,746`), and the model echoed those `[N]` labels into `text`. (2) It invented `§1`/`§2` on a document with no sections — the `§N` notation is baked into the PRD-flavoured persona examples (`evaluatorPrompts.ts:18–28`) and here points at nothing the user can see.\
**Failure mode:** internal-implementation leak into user-facing copy (register violation — evaluator's-eye-view instead of user's-eye-view)\
**Notes:** This is the **same class as UX-017 / OBS-031** (the `Claim #N` leak) but on the surface those fixes never reached. The 2026-07-06 remediation added the "Never name a claim by its index or bookkeeping label" rule to **all four contradiction prompts** (`evaluatorPrompts.ts:92,119,144,164`) and guarded it with a numbered-label lint over produced contradiction/tension messages — but `DOC_LEVEL_SYSTEM_PROMPT` is the one message-bearing prompt that never received the rule, and the lint doesn't cover doc-level output. Prompt-only fix: (a) mirror the anti-index instruction into the doc-level prompt (its `text` fields must quote/restate, never cite `claim [N]` / `block [N]`); (b) forbid `§N` addressing unless the document actually carries numbered sections — refer to content by quote/topic instead. Consider extending the `evalRatchet.test.ts` label-leak lint to doc-level `text` outputs so this class can't reopen on a third surface. Clusters with UX-017, OBS-031, OBS-016.

---

### OBS-035 — doc-level `structure_flow` fires a backwards ordering claim; Block Summaries appear to be fed out of document order

**Date:** 2026-07-08\
**Prompt tier:** doc-level (strong, gemini-2.5-pro)\
**Type flag:** structure_flow (also skews missing_topic — "introduces a problem in §2 and a product in §1")\
**Input excerpt:** `Block Summaries:\n[1] The author introduces a new document-centric writing tool…\n[2] The author argues that over-reliance on chat-based AI … risks degrading … skills.` Returned: _"The document introduces the solution in §1 before establishing the problem it solves in §2."_\
**Expected:** structure_flow reasoning that matches the actual reading order. In the real document the **problem comes first** (the skill-atrophy fear opens the piece) and the **solution comes last** ("I didn't find a tool … so I built one. writtten.com"). So "solution before problem" is factually inverted.\
**Actual:** The model reasoned correctly from the input it was given — but the Block Summaries were not in reading order. **Verified in current HEAD:** `evaluateDocument` sorts summaries **alphabetically by summary text** (`meaningful = summaries.filter(…).sort((a, b) => a.summary.localeCompare(b.summary))`, `src/services/evaluator.ts:703–705`) and claims likewise (`:710`), then feeds that *same* alphabetical order to the model as the positional `[1] … [2] …` Block Summaries (`:760`) and `[N]` Claim Ledger (`:746`). So the doc-level prompt **never sees document order at all** — every `structure_flow`/ordering observation, and every `§N`/`claim [N]` the model emits (OBS-034), indexes an alphabetical list.\
**Failure mode:** false-positive structure_flow — doc-level input is alphabetically sorted, never document-ordered (confirmed, not candidate)\
**Notes:** The alphabetical sort is deliberate but for a *different* purpose — request-hash determinism, so the expensive doc-level pass can be mocked/replayed and dirty-checked (see the comment at `evaluator.ts:700–702`). The bug is **reusing that hash-ordering as the model's positional input.** Fix is therefore not "sort by document order instead" (that would re-break the hash) — it's to **keep the alphabetical order for the `docStateHash`, but build the `Block Summaries` / `Claim Ledger` prompt sections from a separately document-ordered list.** Distinct from OBS-016 (there the summaries were in reading order and the model's ordering call was *correct* but phrased ambiguously); here the input order itself is scrambled. Clusters with OBS-034 (both are the doc-level prompt treating these indices as stable positional anchors), OBS-015 (doc-level anchoring).

---

### OBS-036 — PRD-strictness recurs on a personal essay because the calibration floor never engages when the stage is unset (class = `unknown`); two user dismissals

**Date:** 2026-07-08\
**Prompt tier:** fast (section-eval) + doc-level (doc-quality)\
**Type flag:** unsupported_claim, missing_topic, audience_mismatch\
**Input excerpt:** Personal essay, `Stage/Context: (none set)` throughout. Three separate strictness misfires: (1) section-eval flagged the rhetorical question _"won't my thinking and writing muscles atrophy?"_ as `unsupported_claim` on **three** consecutive evals; (2) doc-quality `missing_topic`: _"The core problem of AI degrading PM skills is asserted in §2 without specific examples or evidence."_; (3) doc-quality `audience_mismatch`: _"The informal phrasing … may not align with a formal business or technical audience."_ — inventing a formal audience that was never set.\
**Expected:** On a first-person essay, a rhetorical apprehension is the content, not a factual claim needing citation; the thesis of an opinion piece doesn't owe "examples or evidence"; and no check should invent a formal audience the author never declared. This is exactly what `document_type_calibration.md` (essay_personal class) exists to relax.\
**Actual:** All three fired at PRD strictness. The user **dismissed** both the atrophy `unsupported_claim` and the `missing_topic` "no examples or evidence" card (confirmed in the archive: `reason: dismissed, actor: user`) — the clearest possible signal they read as noise.\
**Failure mode:** false-positive / PRD-strictness-on-essay — **recurrence of OBS-028 / OBS-023 after the calibration floor shipped (2026-07-05)**\
**Notes:** This is the important one, and it's a *gap in the shipped fix, not a fresh problem.* Two distinct reasons the Phase-6 calibration floor (`document_type_calibration.md`) didn't catch this: **(1) Calibration is inert until the class is known, and the stage was never set.** `classifyDocumentClass("")` → `unknown` → the calibration block is gated to empty for `unknown` (hashes stay stable), so PRD-ish strictness applied. The essay never got the `essay_personal` relaxation because nothing classified it — inference only produced a `suggested_stage` ("Product concept note for internal review") at the terminal `doc-idle`, *after* all the flagging, and it was never accepted. So the floor works only once the class is known, and on a cold essay with no stage it never becomes known in time. Argues for either earlier/auto type-inference feeding the class before the first strict pass (ties `onboarding_first_run.md`), or a less-PRD-strict `unknown` default. **(2) The always-on first-person carve-out didn't fire on a *rhetorical-question* phrasing.** The shipped carve-out lists declarative forms ("I fear…", "I'm concerned…", "I worry…"); _"won't my … muscles atrophy?"_ is the same apprehension in interrogative form and the fast model (gemini-3.1-flash-lite) didn't map it to the carve-out — even though the prompt's own example is nearly identical ("I fear my writing skill will atrophy"). Small prompt extension: cover rhetorical-question and hypothetical framings of first-person concern. Clusters with OBS-028 (near-identical example), OBS-023, OBS-006; feeds `document_type_calibration.md`.

---

### OBS-037 — Rhetorical/hyperbolic emphasis extracted as `fact_claim`, then regenerated as a false `strategic_tension` against the product definition on every rerun

**Date:** 2026-07-08\
**Resolved:** 2026-07-18 — both levers shipped. **Lever 1 (extraction):** `sectionCalibrationBlock` (the three relaxed classes **and** `unknown`, `documentClass.ts`) gains a rule that rhetorical/hyperbolic emphasis and narrative color are not claims of any kind; `prd_spec` stays hash-stable. **Lever 2 (tension):** the four contradiction prompts' NOT-A-CONFLICT bucket (`evaluatorPrompts.ts`) gains the author-reconciled case (A-but-B-so-C = neither bucket) — also the V1-measured "current-state problem vs its own proposed fix" FP on real PRDs. Both prompt edits shift request hashes; the corpus fixtures + demo recording were re-keyed **byte-identically offline** (content-addressed replay, bodies unchanged — ratchet + `exampleReplay.sync` green). Two weak-tier live-recorded discrimination fixtures confirm the behaviour: `rhetoric-extraction` (hyperbole → **zero** claims, asserted at the claim level in `rhetoricExtraction.test.ts`; a genuine "40%" statistic still extracts + flags) and `reconciled-tension` (problem→resolution → neither bucket; `strategic-tension-fraud` still proves a genuine tension fires).\
**Prompt tier:** fast (section-eval, claim extraction) → strong (contradiction / contradiction-sweep)\
**Type flag:** claims (kind) → strategic_tension (also an `unsupported_claim` spawn)\
**Input excerpt:** Rhetorical emphasis in an essay — _"Chat interface was a HUGE thing."_ and _"That's the tipping point for the AI revolution."_ — both extracted as `kind: fact_claim` and written to the ledger. Downstream, across four separate strong calls, the cross-claim check repeatedly manufactured a tension between the product definition (_"a document creation interface where you write everything yourself"_) and _"Chat interface was a HUGE thing"_ — e.g. _"The product vision of users writing everything themselves is in tension with the observation that a 'Chat interface was a HUGE thing'."_\
**Expected:** (1) Rhetorical/hyperbolic emphasis ("a HUGE thing", "the tipping point") is narrative colour, not a citable `fact_claim` to track in the ledger. (2) No `strategic_tension`: the essay *explicitly reconciles* these — chat was big **but** carries a cost, **so** the author built a document-first tool. The narrative resolves the pull; it isn't two live goals fighting.\
**Actual:** The hyperbole entered the ledger as fact, one instance immediately spawned an `unsupported_claim` ("…lacks supporting evidence or industry context"), and the false tension regenerated on every rerun/settle of the block — each strong call (paid gemini-2.5-pro, 7–14s) re-derived a slightly reworded version, which then churned through supersede/re-emit.\
**Failure mode:** claim-extraction over-capture (rhetoric typed as `fact_claim`) → cascading false cross-claim tension + paid-call churn\
**Notes:** Root cause is upstream in extraction: emphatic/rhetorical assertions in non-PRD prose shouldn't be promoted to ledger `fact_claim`s at all — once they are, they seed both `unsupported_claim` (OBS-023/OBS-028 territory) and false `strategic_tension`. Two levers: (a) extraction-side — under relaxed document classes, don't extract rhetorical emphasis / narrative framing as claims (composes with `document_type_calibration.md`); (b) tension-side — a narrative that states A-but-B-so-C has already resolved the tradeoff, which the "NOT A CONFLICT" bucket (OBS-031) doesn't yet cover for *author-reconciled* tensions. The regeneration-per-rerun is the same paid-churn smell as OBS-028 facet 2 / OBS-012 / UX-014. Clusters with OBS-002 (kind misclassification), OBS-025 (tension dedup), OBS-031 (bucket precision).

---

### OBS-038 — Contradiction prefilter surfaces a *compatible* near-duplicate and crowds the *contradictory* claim out of the candidate set — a cross-section hero miss that is candidate-selection, not adjudication

**Resolved:** 2026-07-17 (Phase 8B — `selectContradictionCandidates` in `src/services/prefilter.ts` replaces the whole-section blob query + global top-10 with candidate near-duplicate dedup (≥0.9 Jaccard, keep-first) + **per-claim** retrieval (`perClaimK=5`, `totalCap=15`), keeping lexical Jaccard; swapped into the `"prefilter"` branch of V3's `contradictionCandidates` seam at the contradiction call site in `evaluator.ts`. Deterministic proof: the α/β/γ crowding contrast in `prefilter.test.ts` (γ crowded out under the old blob prefilter, retained under the new selector) + the `contradiction-sla-family` ratchet fixture (β×γ fires end-to-end). Small docs stay byte-identical (self-inclusion: each same-section new claim retrieves itself at Jaccard 1.0). **V3 field re-measure (P04+P09, paid, 2026-07-17) confirms the mechanism scope:** this doc's own β×γ pair (P09 = `vi-04`) **now co-occurs in a prompt and is caught** — recovered, exactly the near-duplicate-occupancy case reported here. The other measured drop, P04's `40%`/`20%` goal-vs-metric pair, is **not** recovered: those claims are lexically *distant* (not a near-duplicate occupancy — ~2 shared tokens), the LEANN/embeddings class, which keep-lexical retrieval cannot close. So the fix closes the near-dup class and isolates the lexical-distance residual as the standing LEANN gate. **Scope note:** selection only; V1 shows adjudication is the larger remaining lever (a separate, deferred item). Embeddings/LEANN stay deferred. See `docs/projects/contradiction_coverage.md` § Phase 8B and `docs/snapshots/2026-07-06_v1_base_rate_corpus_study.md` § 4.)\
**Date:** 2026-07-16\
**Prompt tier:** strong (per-section contradiction check) — but the miss is **pre-prompt**, in the Jaccard candidate prefilter (`prefilterClaims`, `src/services/evaluator.ts:551–554`; `src/services/prefilter.ts`), not the adjudicating model.\
**Type flag:** contradiction (false negative — missed)\
**Input excerpt:** V1 base-rate corpus doc **P09** (a real internal PRD; source text withheld — confidential, referenced by anonymized corpus id only). It carries three latency SLAs that extraction split into distinct ledger claims: **α** "change → *detection* in < 5 min" (real-time NFR), **β** "within < 5 min of a change, *a PR is created*" (end-to-end acceptance criterion), **γ** "*detection* → PR creation in < 1 hr" (PR-cycle NFR). The verified V1 label marks **β × γ** as the strict contradiction (an end-to-end < 5 min promise is incompatible with allowing the detection→PR sub-step up to 1 hr). **α × γ is compatible** (α+γ ⇒ change→PR ≲ 65 min). α and β are lexical near-duplicates ("< 5 min" + "change").\
**Expected:** The contradiction check presents **β and γ together** in at least one prompt so the adjudicator can rule on the real conflict.\
**Actual:** Recall 0 on this doc (paid strong tier: `contradictions: []`; it surfaced an unrelated, plausible tension instead). Diagnosed offline by logging the assembled contradiction-prompt claim sets and replaying the dumped fixtures in mock mode (zero budget): **β and γ never co-occurred in any prompt, in either direction.** When γ's sections (NFR, Success Metrics) settled, the compatible **α** occupied the "< 5 min" slot in the top-10 candidate set and **β was absent**; from β's own section (Acceptance Criteria) the top-10 blob-query never surfaced γ. The judge was only ever shown the *compatible* α × γ pairing — which it correctly did **not** flag. The model never got the chance to miss β × γ; it was never asked.\
**Failure mode:** candidate-selection failure — a compatible near-duplicate of the contradictory claim occupies the lexical-prefilter slot and crowds the true conflict out of the top-K (aggravated by the query being the whole settling section's concatenated claim-blob, which dilutes cross-section retrieval).\
**Notes:** This is a **sharper, more actionable form of the audit's Jaccard-prefilter concern** (`docs/projects/field_validation.md` V3 — the "Q2 vs the second quarter" lexical-distance drop): the danger here is not only *distance* but *near-duplicate occupancy* — when a claim is paraphrased into a compatible and a contradictory variant, the prefilter can systematically retrieve the compatible one and hide the conflict. Because the pair was never co-presented, this run **cannot** say whether the adjudicator would catch β × γ if shown it (distinct from a true reasoning miss). Levers, all in candidate selection rather than the adjudicator prompt: (a) query the prefilter **per-claim**, not per-section-blob; (b) **cluster/dedup near-duplicate claims** before top-K so a compatible paraphrase can't evict its contradictory sibling (shares machinery with OBS-025's tension dedup); (c) **embedding** similarity instead of lexical Jaccard; (d) raise K / widen for numeric-SLA claim families. Surfaced by the V1 P09 smoke run (2026-07-16); feeds the V1 write-up's recall interpretation (a retrieval gap ≠ an adjudication gap) and the V3 prefilter-drop instrumentation. Clusters with OBS-026 (intra-section conflict anchoring drop), OBS-033 (contradiction taxonomy never engages on the pair during typing), OBS-025 (near-duplicate claims from restatement). → see `docs/projects/contradiction_coverage.md`, `docs/projects/field_validation.md` (V1/V3), `docs/projects/evaluator_quality_ratchet.md`.

### OBS-039 — An agent-driven review runs entirely uncalibrated: the skill hands over `stage` but none of the document-type calibration our own prompts carry
...
**Actual:** Neither happens, and (2) is the real defect. The skill mentions `stage` five times and instructs *"Read `stage` first — it tells you what the document is trying to be"* (line 129) — but says **nothing about what to do when it is empty**, and conveys **none of the calibration logic itself**. Our own path spends `evaluatorPrompts.ts` (231 lines) on it: `sectionCalibrationBlock`, relaxed document classes, PRD-grade vs essay strictness, audience-relative jargon suppression. An agent receives a bare string and one sentence.\
**Failure mode:** the agent applies uniform strictness to every document. This is precisely the **OBS-037** class — PRD-grade strictness cascading onto essays/memos, producing false `unsupported_claim` and `strategic_tension` cards on rhetorical or narrative prose — which we diagnosed and fixed on our own prompts in July and have now reintroduced through a second engine. Related: the jargon noise V1 measured (21–53 flags/doc on real PRDs) was fixed by audience-relative calibration the agent also never receives.\
