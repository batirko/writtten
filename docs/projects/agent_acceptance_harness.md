...
**Closed 2026-06-01; app Phase 2 additions landed 2026-06-02; app Phase 3 additions landed 2026-06-02.** All harness Phase 1 and Phase 2 todos shipped and verified. Phase 3 additions: `getState()` now includes `sessionStats` (`fastCalls`, `strongCalls`, `totalCalls`, `totalLatencyMs`, `avgLatencyMs`) from `llmLogger.getSessionStats()`; `data-testid="api-key-input"` and `data-testid="settings-panel"` on BYO-key settings UI; `data-testid="session-stats"` on debug-panel cost row; `data-testid="arrival-indicator"` on batched-arrival chip. Prefilter is a pure function with no LLM calls — tested directly in `src/services/prefilter.test.ts`, no mock-mode fixture needed. Phase 4 harness exit-criterion in `docs/plan.md` is the next forward reference.
