...
- [x] **C9 — Overlapping / co-located-highlight targeting** — **shipped.** Reverse hover now resolves the covering _set_ by coordinate (`posAtCoords` → `computeObservationRanges` hit-test in `Editor.tsx`, reusing the highlighter's offset→pos mapping), primes **all** covering cards (`spanFocusRelatedIds` → SidecarFeed un-dims the co-covering set, symptom ii), and picks the **smallest range as primary** so a nested substring is reachable (symptom i) (§ C9). A downgraded invisible-anchor substring is targetable inside a visible span; plain text covered only by invisible anchors stays inert (surfaced-only cue). `resolveCoveringSet`/`computeObservationRanges` are unit-tested; cross-claim dual-span preserved.
