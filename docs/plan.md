...
- [ ] **Discomfort-budget ceiling (G4)** — decided 2026-06-17: **floor + ceiling hybrid** — guarantee the top contradictions are always visible (floor) but cap visible contradictions at `CONTRADICTION_CEILING` (default 3), overflowing extras into "also noticed" under an explicit "N more contradictions" signpost. Reconciles a code/doc drift (the shipped feed is a uniform cap with no contradiction floor/ceiling). Small bounded change to `partitionFeed` + a labelled drawer section. → see `docs/projects/philosophy_guardrails.md` (G4) · R6.3 — 🟢 Low (build) · 🔧
