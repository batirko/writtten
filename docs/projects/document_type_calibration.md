...
- [ ] **Rhetorical-question carve-out (OBS-036 facet 2)** — extend the fast-tier first-person carve-out in `MERGED_SYSTEM_PROMPT` so interrogative/hypothetical framings of a personal concern ("won't my writing skill atrophy?") are carved out alongside the declarative "I fear…/I'm concerned…/I worry…" forms. Add a discrimination fixture. Prompt-only, always-on (independent of class).
...
- [ ] **Infer the class before the first strict pass, or soften the `unknown` default (OBS-036 facet 1)** — today an un-staged non-PRD gets full PRD strictness on the way in because inference only produces a `suggested_stage` at the terminal `doc-idle`. Either feed early type-inference into the class before the first section-eval (ties `onboarding_first_run.md`), or make the `unknown` default less PRD-strict (keep contradiction/clarity/jargon on; relax `unsupported_claim`/`missing_topic` toward the essay/comms floor until a class is confirmed). Decision needed — the un-staged case is the common one, not the exception.
