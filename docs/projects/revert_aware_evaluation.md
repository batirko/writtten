...
**Done. Mechanism 2 (snapshot/restore) shipped 2026-07-04; Mechanism 1 (section-boundary commit debounce) shipped 2026-07-06.**

Mechanism 2 — `src/services/evalSnapshot.ts` + the restore path in `evaluateSection` (`src/services/evaluator.ts`). Verified both by unit test (`src/services/evaluator.test.ts` § "revert-aware snapshot restore") and live in the browser via the dev harness: a toggle→click-away→click-back→revert sequence that previously left a stray observation now returns the feed to exactly its pre-toggle state (0 active observations), with the transient card honestly closed into the archive rather than lingering or silently vanishing.

Mechanism 1 — a **committed** section-boundary layer over the always-live `resolveSections()` result, entirely in `src/editor/Editor.tsx` (`onUpdate` step 2d) + pure helpers in `src/editor/section.ts` (`sectionOwnerMap`, `hasStructuralChange`, `changedSectionIds`). A re-sectioning (a *surviving* block changed owner — the fingerprint that distinguishes a heading toggle from typing/Enter-split) freezes the committed boundaries and suppresses eval dispatch for the affected sections until the change survives `EVAL_DEBOUNCE_MS` (3 s). A toggle→revert within the window nets **zero** section evals; a *sustained* heading commits and fires a synthetic settle for the new section shape (same Invariant-#4 gates). Detection reuses the `resolveSections` walk `onUpdate` already runs — no new LLM scan (invariant #3). See `docs/mechanics/evaluation-triggers.md` § "Section-boundary commit debounce". Verified live via the harness event stream (toggle→revert now emits 0 new `settle`/`request` events and 0 stray observations) and by `src/editor/section.test.ts` unit tests for the detection helpers.

**Formerly-known gap — now closed:** an in-flight eval for a still-transient section whose boundary reverts mid-flight is now invalidated. Mechanism 1 calls `invalidateSectionEval(sectionId)` (`src/services/orchestrator.ts`, bumping `sectionEvalGeneration`) whenever a re-sectioning is detected or reverts, so a late write for a stale boundary skips its post-LLM writes — the same generation machinery `block-removed` uses.
...
| Phase | Contributes                                                                                                                                                                                                                                                                                                                    |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **6** | **Mechanism 2 (content-hash snapshot/restore):** an eval result cache keyed by normalized-text hash; return to a known hash restores the cached observation set, instead of tearing down and rebuilding. **Mechanism 1 (section-boundary commit debounce):** a committed-boundary layer that suppresses eval dispatch for a re-sectioned area until the change survives a 3 s settle window — so a toggle→revert fires zero calls — and invalidates in-flight evals for reverted boundaries (closing M2's known gap). Both shipped. |
...
- [x] **Cancel superseded in-flight evals.** **Shipped with Mechanism 1 (2026-07-06).** `invalidateSectionEval(sectionId)` in `src/services/orchestrator.ts` bumps `sectionEvalGeneration` (so `isLive()` goes false and a late write is skipped) and drops any queued coalesce/rerun for the section. Called from `Editor.tsx` whenever a re-sectioning is detected or reverts — broadening the generation bump from "block removed" to "boundary changed / reverted", exactly as § Confirmed against code anticipated.
...
### Mechanism 1 — section-boundary commit debounce — shipped 2026-07-06

- [x] **Structural-change detection.** `sectionOwnerMap` / `hasStructuralChange` / `changedSectionIds` in `src/editor/section.ts` — a *surviving* block changing owner is the re-sectioning fingerprint (distinguishes a heading toggle from typing / Enter-split). Pure, unit-tested in `src/editor/section.test.ts`.
- [x] **Committed-boundary layer + settle debounce.** `Editor.tsx` `onUpdate` step 2d: a detected re-sectioning freezes the committed owner map, suppresses dispatch for the affected sections (guards at all three §1–§3 triggers), and arms a `structuralTimer` for `EVAL_DEBOUNCE_MS`. `resolveSections()` stays always-live for anchoring/rendering — only eval-trigger dispatch is gated.
- [x] **Revert → zero evals; sustained → commit + synthetic settle.** Return-to-committed within the window cancels the timer (no eval). A change that holds commits the new boundaries and fires one gated `block-settle-pause` per affected section (so a sustained heading is evaluated; Invariant #4 gates preserved).
- [x] **In-flight invalidation.** `invalidateSectionEval` bumps the generation for each affected section on detect/revert — closes M2's known in-flight-revert gap.
- [x] **Committed-map seeding.** Load-on-mount, `loadDoc`, `importContent`, bulk paste, and clear all call `commitOwnersNow` so their own dispatches aren't misread as re-sectionings.
- [x] **Tests + live verification.** Unit: `section.test.ts` (toggle = structural; type/split/remove ≠ structural; heading-delete = structural). Live: harness event stream shows a toggle→revert now emits 0 new `settle`/`request` events and leaves 0 stray observations; a sustained heading fires one synthetic settle.
...
- **`evaluateSection` already skips the model call on unchanged text** (in `evaluator.ts`), but overwrites the block-summary during the transient and never restores closed observations — so Mechanism 2's real work is the observation snapshot/restore, on a **dedicated** store, not a reuse of the block-summary hash. This was confirmed independently of the M1 reproduction and stands unchanged: the intro section's stored hash genuinely changes across the toggle (different membership → different `combinedText`), so the summary hash can't serve as Mechanism 2's key across a re-section — a dedicated snapshot store keyed on normalized text is still the right design.
