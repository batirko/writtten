...
**Observation taxonomy under test** (9 types, fixed — Phase 4 added `strategic_tension`):
`clarity`, `contradiction`, **`strategic_tension`**, `unsupported_claim`, `undefined_jargon` (span scope) · `missing_topic`, `underexposed_topic`, `audience_mismatch`, `structure_flow` (document scope).
