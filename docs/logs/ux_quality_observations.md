...
**Interaction:** On the nth bullet, cleared it and pressed **Backspace** on the empty item, expecting to drop back to a normal paragraph.\
**Expected:** Empty bullet + Backspace → exit the list into a top-level paragraph.\
**Actual (before fix):** Backspace merged the empty item into the **previous** item as a second paragraph — `<li><p>two</p><p></p></li>` — an invisible stray block inside a list item. A second Backspace removed it and left the cursor at the end of "two", still inside the list. There was no way out via Backspace at all; you cycled.\
**Failure mode:** two layers. Without `ListKeymap` there is no Backspace list handling whatsoever, which produced the stray nested paragraph. Adding `ListKeymap` fixed _that_ (the item is now cleanly removed) but still doesn't **leave** the list — it drops the cursor at the end of the previous item. The lift-out needed an explicit handler.\
**Notes:** Enter on an empty item already exited correctly the whole time — that's `splitListItem`'s built-in behaviour — so the list had exactly one exit, and it wasn't the key most people reach for. `ListEscape` (priority 1000, above ListKeymap's 100) lifts an empty item via `liftListItem`: top-level → paragraph after the list; nested → outdent one level. It declines for non-empty items, text selections, items with a nested list hanging off them, and anything outside a list, so ordinary Backspace is untouched. Worth noting the guard test is a **unit** test rather than a browser one on purpose: plain character deletion is native editing rather than a keymap handler, and synthetic key events don't reach ProseMirror for it — the browser harness cannot exercise this class of behaviour.

### UX-025 — Formatting hotkeys reported dead; bindings verified live, cause is upstream of the page (unreproduced)

**Date:** 2026-07-20\
**Status:** **could not reproduce in-app** — needs the reporter's environment before anything is changed.\
...
**Notes:** Two answers would close this: which browser/OS, and whether **Cmd+B** (bold) and **Cmd+Alt+1** (heading) also fail. If Cmd+B works and Cmd+Shift+8 doesn't, it's layout or interception and the fix is an alternate binding, not a keymap repair; if all fail, it's focus or something app-side not yet found. Overlaps **UX-004** (_Editor formatting affordances_, already in the remediation Todo): there are no visible formatting controls, so hotkeys are the only path to these transforms and their failure is both invisible and unrecoverable. Whatever the root cause here, UX-004 is the thing that would have made it a non-event — the slash menu exists but doesn't cover list/heading transforms of an existing paragraph.
