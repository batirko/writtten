...
**The permission _state_ is readable even though the dialog is not — measured 2026-07-21, and it reopens a design space this section had closed.** The sentence above ("a permission dialog is browser chrome that no automation can observe") is true and stays true, but the conclusion drawn from it elsewhere — that no allow/deny-aware behaviour can be built — does not follow. In **real Chrome 150 on `https://writtten.com`**, `navigator.permissions.query({ name: "local-network-access" })` resolves with a meaningful state (`granted` on a machine that had granted it), while `geolocation`/`notifications`/`camera` in the same call returned `prompt` — those controls are what make the reading trustworthy. `PermissionStatus.onchange` is present. **Two traps recorded so the next session does not repeat them.** First, the same query run inside the **Electron-based desktop browser** returned `denied` for _everything_, controls included: that is a force-denying shell, not data, and reading design intent off it would have been the same class of error as concluding "no prompt appeared" from a screenshot. Always include a known-`prompt` control in the same call. Second, **`local-network-access` and `local-network` are both valid `PermissionName` values** in Chromium 148/150 and returned different states in the same call — which one actually gates loopback is unconfirmed, and conditioning on the wrong one fails silently. Invalid names throw a distinguishable `TypeError` ("not a valid enum value of type PermissionName"), so name validity is cheap to re-test. ~~**Firefox is entirely unmeasured** and its narrower name set probably lacks this permission~~ — **measured 2026-07-23, and that guess was wrong; see below.** The conditional pre-flight this unlocks is specced on the Phase 8 milestone (`docs/plan.md`).

**Both engines expose it, under opposite names — measured 2026-07-23, and this corrects the paragraph above.** Real Chrome 150 and real Firefox 152 on `https://writtten.com`, each reading validated by a `geolocation`/`notifications`/`camera` control row in the same call:
...
Firefox does **not** lack the permission; it names it `local-network`. The prediction that it would fall to the fallback branch was desk reasoning of exactly the kind that has already shipped wrong twice here, and it is recorded rather than quietly deleted because the pattern is the lesson. Consequences now in code (`agentLocalNetworkPermission.ts`): the app queries an **ordered candidate list** rather than one constant, `local-network-access` first — on Chrome the other name reads `prompt` while loopback is genuinely `granted`, so the order is the difference between silence and a pre-flight at every connect.

**Which name gates loopback in Firefox is still open.** `local-network` resolving is not evidence it tracks the grant; the § _Landing verification_ record has the owner allowing Firefox's prompt on 2026-07-20, yet it reads `prompt` today. Until a Firefox connect-and-allow is seen to flip it to `granted`, it is treated as unreadable, and Firefox keeps the old unconditional warning. **Settle it at the next Firefox live-check** — one console paste before and after allowing.

**A third trap, found while building and now guarded in code.** The Electron-based browsers inside AI coding apps — which `/agent` recommends *first* since #254 — return `denied` for every permission including all three controls. The app now re-checks the control row before believing a `denied`, because a shell can deny the Permissions API while still allowing the loopback fetch, and acting on it would refuse a pairing that might work. This is the same control-row discipline this section already prescribes for manual measurement, moved into the product because the failure landed on a recommended path.
...
**Safari:** unsupported by design and refused before the first probe (UX-027) — and, since 2026-07-21, **never offered in the first place** (UX-044). Refusing at the probe was still one level below the choice: the welcome modal, the keyless banner, and the Engine tab each offered the path on the flag alone, and taking it handed the agent the eval slot, pausing the built-in evaluator for the rest of the session against a slot WebKit can never serve. All three now gate on `agentPathOffered()` (`src/services/agentOffer.ts`), and the Engine tab, while still visible, is never selectable on WebKit — it explains itself on hover or tap and routes to `/agent/`, which already existed. See `docs/mechanics/agent-bridge.md` § _Browsers never offered the path_.
...
- **Lifecycle:** user dismissal works identically (writes the same `DismissalSuppression`, so re-submissions are filtered — G1 applies unchanged). Anchor deletion collapses the card via the existing highlighter path. The **evaluator never auto-closes external cards** — every reconcile arm (`evaluatorReconcile`, `docReconcile`, the sweep's blanket close) filters on `!obs.source`; they are not the evaluator's to close. The agent may `retract` what it submitted **this session** — an earlier run of the same agent is refused too (→ `auto_closed`, closure reason `retracted`, archived as "withdrawn by \<name\>": who closed it, never why, since the user may have asked for the cull and writtten never sees that conversation; owner 2026-07-25, reasoning in `docs/mechanics/agent-bridge.md` § _Retract and revoke_). Disconnect: persist quietly (decision 7).
...
### What the user is told they can ask for (2026-07-24)

Every capability this spec built for the *user* was documented only for the *agent*. Steering, lenses, and an agent's permission to withdraw its own cards all shipped and worked, and all three lived in `docs/skills/writtten-agent.md` — a file the user pastes without reading and never sees again. The connected panel reported `127.0.0.1:<port>`, a cadence, and one line about the engine pause. **The finding is that the owner did not know these existed three days after shipping them** (UX-043); a first-run user had no chance. Shipped capability nobody can find costs the build and returns nothing.

**Home (owner, 2026-07-21): the connected state, with `/agent` as the deep read.** Two alternatives were declined and the reasons don't expire. The **agent's own end-of-pass report** costs no app UI and is the surface a user reliably reads — but it is agent-asserted text the boundary cannot enforce, and it exists only *after* a pass, so it cannot inform someone deciding what to do with a connection they just made. The **first-run path** would fight the welcome modal's trim (UX-042), decided in the same session.

**The copy is three things a person would say, not three feature names** — the same reason the skill's lens examples work. "Steering", "lenses" and "retraction" are our vocabulary; a user says *"check whether the metrics section holds up"*. `agentCapabilities.test.ts` enforces this mechanically (lowercase opening, no internal vocabulary, at most three), because a constraint that lives only in a comment dissolves one well-meaning edit at a time.

**The structural decision worth carrying: the panel and the skill are pinned to each other.** Each ask names the skill section that makes it true and the test asserts that section exists. This matters because the drift failure is silent — the panel keeps advertising, the agent no longer honours, the user asks and gets nothing, and no test or log reports a fault. It is also why the user-facing half and the skill's retraction gap (the curation milestone's piece 1) were built in **one pass**: two descriptions of one capability, written separately, drift by construction.

**The fifth capability, and the honest edge.** A connected agent arrives with its working directory — the repo it has open, the docs it has read — while writtten's own critic sees the document and nothing else. That is the difference between *this claim isn't supported here* and *this claim isn't true*, and it makes **the folder you connect from the one real quality lever on this path**. Framed as reach rather than configuration, with the edge attached: context your agent has is context your reader doesn't, so something it accepts as settled can still land unsupported for the person you are writing for.
