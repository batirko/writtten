...
| [evaluator_quality_ratchet](projects/evaluator_quality_ratchet.md) | idea | 4 | Labeled fixture corpus + two-tier scorer (deterministic replay CI + opt-in live precision/recall) so evaluator accuracy can't silently regress. Prerequisite for SkillOpt prompt optimization. |
...
- [ ] **Evaluator quality ratchet** — the SkillOpt labeled eval fixture set (`src/services/eval-fixtures/`, ground-truth docs) wired into Vitest, so recommendation accuracy can't silently regress as prompts change. → see `docs/projects/evaluator_quality_ratchet.md` · `docs/projects/ai_tooling_integration.md` (Phase 2 carry-over)
