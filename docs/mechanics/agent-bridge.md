...
- **Settings** carries one plain line above the engine help: *"Nothing is reading your document yet."* Same sentence for both engines on purpose; the missing precondition differs, the consequence does not. The trailing *yet* is load-bearing: the line renders directly above the engine help, which says a coding agent (or a model provider) does the reading, so without it the two sentences read as a contradiction rather than as a state and its description.
...
The permission watcher lives **through the wait**, not just while the callout shows. The real dialog only appears _after_ the probe starts (i.e. after the callout has closed), so a user who clicks **Block** on it must still be caught: `local-network-access` flips to `denied`, and the watcher stops probing and raises the blocked callout instead of looping into the 25 s floor. The same watcher **auto-continues** the moment the user allows in site settings — no button to hunt for, and it reattaches the _existing_ pairing (an agent may still be on that bridge) rather than minting a new token.

**Resuming a stored pairing consults the permission too.** A pairing persists across reloads (decision 7), and the resume-on-mount path branches on the queried state exactly like a fresh connect — `denied` → blocked recovery, `prompt` → explain first (reattaching would raise the dialog cold again), `granted`/unreadable → reattach straight through. The first build reattached blind, so a permission **blocked between sessions** resumed straight into a probe that could never answer, with nothing on screen and `permissionRef` empty so even the live watcher couldn't see it. That is the specific shape of "the app doesn't know the site is blocked," and it is now closed (`useAgentBridge.resume.test.tsx`).
...
**Retract** — `retractExternalObservation(id, sessionId)`, driven by the bridge's `retract` event. Closes with reason `retracted`; the archive drawer renders that as **"withdrawn by \<name\>"**. Refuses, writing nothing, when the observation is missing, already closed, belongs to another session — including an earlier session of the same agent — or is native. That last case matters: an agent guessing an id must not be able to close writtten's own findings.

**What an agent is told it may do with this (2026-07-24).** The endpoint was documented from the start; the *permission* was not, so the 2026-07-20 dogfood session inferred it and withdrew four of seven cards on its own reading. `docs/skills/writtten-agent.md` § 4 _Withdraw what no longer holds_ now states it: an agent may withdraw its own findings **on its own reconsideration or because the author asked it to** — so "leave the three that matter most" is carried out as a sequence of retractions across its own pass. The boundary it also states is the one the code has always enforced, said out loud for the first time: **ownership, not motive.** For a card it did not write, the agent answers *"which of these matter most, and why"* in its own session and the author does the dismissing — because dismissal writes the suppression that teaches and a retraction writes nothing, so a delegated cull would remove the feed the author was going to judge and teach the system nothing in exchange. Pinned by `agentPrompt.test.ts` § _the retraction permission_ (survives a prompt rewrite) and `agentCapabilities.test.ts` (the connected panel advertises this ask, so the skill section backing it must exist).

**What the archive says happened (2026-07-25).** The drawer used to file a withdrawn card as **"retracted by \<name\>"**, which tells the reader the agent changed its mind. That is false whenever the user was the one who said "leave the three that matter most", and it is not a gap we can close by asking: writtten never sees the conversation that produced the withdrawal, so a "the user asked me to" flag on the retract frame would be the agent's own word — unverifiable exactly like `agentName`, in the surface a user opens to find out why a card is gone. The label now says what the app can stand behind and stops there: **"withdrawn by \<name\>"**, who closed it and never why. Rejected: rendering that flag as reported speech ("says you asked"). It buys a story we cannot check, gives the agent a reason to always assert it, and costs a bridge-script edit — `handleRetract` rebuilds the frame rather than relaying the body wholesale, so unlike the `changedSections` precedent this direction does not ride for free. The closure **reason** stays `retracted` on the wire and in the debug log, where the word names a mechanism rather than addressing the user; only `closureLabel.ts` changed.

**May a pass close a _prior_ session's cards? No (owner, 2026-07-25).** Under engine exclusivity "yesterday's pass left junk" is the ordinary case rather than an edge one, and this is not guardrail territory — no user judgement is being overridden, it is the same engine's own stale output. It stays refused on two grounds. Lifting the `sessionId` check alone would change nothing an agent could act on, because it holds no ids for older cards; a real yes means putting ids in the snapshot, which is the one convenience that dissolves the wall described next. And `sessionId` is already what attribution and revoke scope on — a cross-session close would let a fresh pairing clear cards the user need not associate with it, while the revoke confirm went on counting one run's. Stale cards from an earlier pass are the user's to dismiss, and the agent can say which ones it no longer stands behind.

**The wall is two facts, and until 2026-07-25 neither stated it.** "An agent cannot close a card it never wrote" holds because `AGENT_OBSERVATION_FIELDS` (`agentSnapshot.ts`) ships no observation ids — the only id an agent ever holds is one returned by its own accepted submission — and because of the single ownership check in `retractExternalObservation`. The two sit in unrelated modules and referred to each other nowhere, so either could have been changed on its own merits. Adding ids to the snapshot "so the agent can reference cards precisely" is a reasonable-sounding convenience that would demote the check from a wall to guess-resistance, with the whole suite green. Each half now names the other in a comment, and both are pinned: _the wall — an agent cannot close a card it never wrote_ in `externalObservationLifecycle.test.ts`, one refusal per out-of-reach population (another agent's card · the same agent's earlier session · a native card), and _the wall — an agent can only name cards it wrote_ in `agentSnapshot.test.ts`, which asserts the allowlist carries no identifier.
...
`agent` is deliberately **not** in the logger's `LIFECYCLE_TYPES` retention bucket: those get evicted first, and bridge events are the whole evidentiary record.

**They are not, however, low-frequency — watch mode made the heartbeat the bulk of the log (UX-050).** A parked agent re-reads on a 60-second cycle whether or not the document moved, and each read wrote a row into the same 120-row budget as the submissions: a real session exported 40 agent rows of which 31 were `pull` and `snapshot` bookkeeping and 8 described the pass, and two hours of watching would have evicted every submission in it. So `logAgent` **collapses an unbroken run of polls at one document version into a single row**, carrying `repeats` and `lastAt`; anything the agent actually does lands between polls and starts a new run, which is what keeps the sequence readable as "read N times, submitted, read again". The run breaks on a version bump too, so the row can never claim reads of a document that had already changed.

**A pairing row survives a document clear, marked `carried`.** Clear workspace, import, and loading the example all call `llmLogger.clearLogs()`, and none of them touch the bridge — so the log lost the one row identifying which engine was speaking while that engine kept working (UX-051). The logger keeps the last pairing transition outside the buffer and re-emits it after a clear when the state is still `connected`; `carried: true` stops it reading as a fresh connection, and the session id is filled in from the most recent agent row, because the pairing row is written when the stream opens — before `hello` names the run.

**A `submission` row carries the anchor's size, never its text** — `anchorChars` / `anchorWords`, plus `conflictingAnchorWords` when a conflict-type submission named its second passage. Anchor *relevance* is the one property of a submission the boundary deliberately does not judge (it verifies the quote resolves, not that it is the passage the observation is about), so a card anchored on its own clause and one anchored on the whole paragraph around it were indistinguishable in the log (UX-052, and the measurement OBS-042 needs). Sizes preserve the no-prose property that lets this family ship to production; the absence of `conflictingAnchorWords` is itself the measurement of how often a two-sided type arrives one-sided.
...
**That collapse is now split (2026-07-24, UX-053).** `watching` had gone on meaning both *holding the whole-document read back* and *read it, nothing to raise*. Below the band the status row now reads `holding off`, returning to `watching` the moment the band moves; the feed's empty state names which quiet it is (`below-threshold` / `awaiting-pickup` / `read-nothing-found`) via `feedQuietView`. The phrase is ranked below `stalled`, `pending > 0` and `!engineReady`, and never replaces `reading` or `awaiting pickup` — an agent below the band is genuinely reading now that the band splits the pass. An agent in the `quiet` or `none` phase is deliberately **not** called a deliberate hold: it may simply have wandered off, and promising a critic that is not there is the overclaim `engineReady` exists to stop.
