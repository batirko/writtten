...
- [ ] **Infer the class before the first strict pass (OBS-036 facet 1 — the richer-inference alternative)** — the *softening* half of this item shipped in Phase 6 (see Phase-6 todo: the `unknown` default is no longer PRD-strict on `unsupported_claim`/`missing_topic`). What remains for Phase 8 is the stronger option: feed early type-inference into the class *before* the first section-eval so an un-staged doc is classified on the way in rather than defaulting to the softened `unknown` floor (ties `onboarding_first_run.md`).
