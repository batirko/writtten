...
| -------------- | -------------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--font-serif` | **Faustina** (variable)                            | Editor canvas body + headings, archived "ghost anchor" quotes | **Decided 2026-07-05 (prototype-matched).** A soft-contrast, high-x-height humanist serif that reads _light and easy_ on screen — matched against Anthropic's editorial serif in a live side-by-side. Chosen over Source Serif 4 / Spectral / Lora (higher-contrast book serifs that read "heavy") and Newsreader (the face the parallel session tried and rejected as too heavy → its temporary "go sans-only" detour is superseded: the problem was Newsreader's contrast, not serif-as-such). Self-hosted via `@fontsource`. |
