...
- [x] **Dismiss + Undo toast** — **built.** Dismiss is optimistic (the card animates out via the R3c `cardExit`), then a transient "Dismissed · Undo" toast rides the bottom of the feed column (`.undo-toast`, sticky, ~5s auto-fade). Undo restores the whole group (`reactivateObservation`, same ids, no archive churn) **and** rolls back each dismissal suppression (`deleteDismissalSuppression`, `db.ts`) so an accidental dismiss never silently trains the feed quieter (G1). `handleDismissObservation` now returns the suppression id it wrote so the toast can reverse it; the group dismisses as one unit and Undo reverses them together. See `docs/mechanics/dismiss_undo.md`. (§ C3)
...
**Shipped.** The dismiss now animates the card out (R3c `cardExit`), writes each dismissal (collecting the suppression ids), and shows one `.undo-toast` ("Dismissed" + a quiet "Undo" text-button) pinned to the bottom of the feed column for ~5s. `App.handleDismissObservation` returns the id of the suppression it wrote; `App.handleRestoreDismissed` reverses a group by `deleteDismissalSuppression` + `reactivateObservation`. The feed dismisses/undoes a whole same-span group as one unit. The `showMark`/highlight path is untouched. The behavioural spec lives in `docs/mechanics/dismiss_undo.md`; the design contract below is unchanged.
