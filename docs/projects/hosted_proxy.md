...
| Phase | Contributes |
| ----- | ----------- |
| **6** | (If GO) A single serverless proxy endpoint holding the owner's paid Gemini key, a hard global daily spend cap + per-client throttle, the "N free evals / day, then BYOK" client UX, the plan.md decision log entry, and honest privacy copy. Gemini-only (the only free-tier-relevant provider). |
