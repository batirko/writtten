...
**Date:** 2026-07-21\
**Status:** open — **no design settled**; the direction below is the owner's framing, not a decided mechanism.\
...
**Direction:** unsettled, and deliberately not pre-decided here. The question is where a *user-facing* statement of "here is what you can ask it for" belongs — the connected state in the panel, the `/agent` page, the first-run path, or the agent's own opening report back to the user, which is the one surface the user reliably reads and the only one that costs no app UI at all. Whatever the answer, it must not become a feature list: the lens examples in the skill are good precisely because they are phrased as things a person would actually say.\
