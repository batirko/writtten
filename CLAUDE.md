...
`docs/plan.md` is the source of truth for the current phase — check its "Current phase" marker (line 7) before adding functionality. As of 2026-07-10 the current phase is **Phase 7 — "Later / optional (post-traction)"**: Phases 1–4 are implemented, Phase 5 (egress/install/hardening) is essentially complete, and **Phase 6 — "Make it good: experience & signal quality" — closed 2026-07-10** with its committed scope shipped (visual + companion-surface feed, UI/UX mechanics, onboarding/first-run activation, mobile courtesy pass, maturity-aware severity, register discipline, fast/strong signal precision). Its four milestones still open at close — contradiction coverage (OBS-033), pre-public review, hosted demo, hero visual — plus a new _strong-tier eval reliability_ infra item and the validation track (V1–V3, deferred 2026-07-07) were all moved into Phase 7 (see `docs/plan.md` → Phase 7, incl. _Carryover from Phase 6_). Phase 7 is post-traction / later work — **don't pre-build it**; treat open items as parked until scheduled. Don't trust this paragraph over `docs/plan.md` if they ever drift — update the plan there, then mirror the headline here.
