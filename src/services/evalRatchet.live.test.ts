...
function mergeTypeScores(
  ...
  for (const fp of result.falsePositives) {
    const isKnownGap = (fixture.knownGaps ?? []).some(
      (g) =>
        g.type === fp.type &&
        (g.sectionId === undefined || g.sectionId === fp.blockId),
    );
    if (!isKnownGap) {
      const k = fp.type;
      const s = acc.get(k) ?? { tp: 0, fp: 0, fn: 0 };
  ...
  for (const fn_ of result.falseNegatives) {
    const isKnownGap = (fixture.knownGaps ?? []).some(
      (g) =>
        g.type === fn_.type &&
        (g.sectionId === undefined || g.sectionId === fn_.sectionId),
    );
    if (!isKnownGap) {
      const k = fn_.type;
      const s = acc.get(k) ?? { tp: 0, fp: 0, fn: 0 };
...
describe.skipIf(!LIVE)(
  "Evaluator quality ratchet — Tier 2 (live prompt scorer)",
  () => {
    ...
    for (const fixture of corpus) {
      it(
        `live: ${fixture.id} — ${fixture.description}`,
        async () => {
          ...
          if (gaps.length > 0) {
            ...
            for (const g of gaps) {
              ...
              const status = misses.length > 0 ? "❌ still misses" : fires.length > 0 ? "⚠️  still FPs" : "✅ resolved";
              console.log(`  ${status}: ${g.type}${g.sectionId ? `@${g.sectionId}` : ""} — ${g.note ?? ""}`);
            }
          }
        },
    ...
    it("scorecard summary", async () => {
      ...
      expect(aggPrecision, `aggregate precision (${(aggPrecision * 100).toFixed(1)}%) below floor 0.6`).toBeGreaterThanOrEqual(0.6);
      expect(aggRecall, `aggregate recall (${(aggRecall * 100).toFixed(1)}%) below floor 0.7`).toBeGreaterThanOrEqual(0.7);
    }, 5_000);
  },
);
