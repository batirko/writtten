...
> **V1 readiness update (2026-07-16).** The validation prerequisite this milestone was gated on is now mostly built: the V1 base-rate machinery exists (`docs/projects/field_validation.md` — a 28-doc hand-labeled corpus incl. 6 real author-written PRDs/case studies, and the resumable keyed runner `npm run eval:v1`), so step 1 below no longer means "assemble docs" — it means **ride the V1 corpus**: add the strong-model margin-notes pass as a second recorded run over the same docs and diff against the labeling sheet. Two things V1 Run 1 (2026-07-16, 9-doc subset) already teaches this milestone: **(1) sequencing** — wild precision on the *existing* types is poor (contradiction 15%, `undefined_jargon` 21–53 flags/doc on real PRDs), so adding five new types now would multiply a noise problem; expansion stays downstream of the Phase-8 calibration fixes (OBS-003/005, OBS-038) *and* a fuller V1 run, not just "V1 ran once". **(2) prompt-bloat is confirmed as the binding shape constraint** — the fast call already under-adjudicates on the free tier, so any confirmed new span types should lean doc-level/strong-call (or the client-side route below) rather than widening the merged fast call.

- [ ] **Validate before writing prompts — on the V1 corpus.** Run a strong model over each V1 corpus doc collecting *all* margin notes a senior PM reviewer would write (one extra recorded pass; the runner and doc fixtures exist). Categorize the residual notes that map to no existing type; the residual pile is the empirical candidate list. Adjust the candidates below based on findings. Cheap first cut before any new run: re-read the **existing** V1 Run 1 transcripts for notes the pipeline emitted under the *wrong* existing type (misclassification residue is candidate-type evidence too).
- [ ] **`ownerless_commitment` goes client-side first.** The ledger already carries `commitment`-kind claims; prototype the no-LLM regex/heuristic scan (see § Open questions) against the V1 corpus's recorded ledgers and measure its false-positive rate before considering an LLM version. If it holds, it ships as a Milestone-D-style derived check — zero request cost, immune to the prompt-bloat constraint.
