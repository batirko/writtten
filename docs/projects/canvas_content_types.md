...
**In progress — Phase 6 floor shipped; Phase 9 (images/task-lists as real nodes) remains.** The Phase 6 supported-set decision, the degradation mechanism, and the eval-model interaction were locked 2026-06-18 and are now built: editable tables (eval-inert), a slash-menu "Table" insert, and image paste-degradation. What remains (Phase 9) is richer authoring — images and task lists as real nodes, gated on the local-first storage story.
...
- [x] **Eval-model interaction** (§). A table is **inert** — it carries a `blockId` (section resolution intact) but its cell text is **excluded from `combinedText`** (`section.ts` `buildCombined` skips `isTable` members) **and from the body check** (`evaluator.ts` `hasBody` excludes `isTable`, so a heading+table-only section stays bodyless and doesn't hallucinate). No claims from cells, no span checks inside cells. Doc-level table→prose context deferred to Phase 9.
...
**Settled 2026-06-18.** Bias toward **the PM persona's real documents**, not editor completeness. Phase 6 adds **tables + links** as real editable nodes and degrades everything else; images + task lists are Phase 9. A PRD/spec/decision-doc routinely contains:
...
| **Task / check lists** | Med — action items, acceptance checklists                     | Low–Med                                                          | Phase 9, unless dogfooding shows demand.                                          |
| **Images / diagrams**  | Med — screenshots, flow diagrams                              | High (local-first storage + egress decision)                     | Phase 9, gated on the storage decision.                                           |
...
- **Open question, deferred to Phase 9:** should table cell text feed _doc-level_ context (a metric in a table is exactly the kind of thing a `contradiction` should catch against prose)? Powerful but harder (needs structured extraction, not a flat blob); revisit with corpus evidence (`field_validation.md`).
- An **image** (Phase 9) is fully inert (no text). Until then images degrade per the contract below.
...
- An `<img>` → a fenced/inline placeholder carrying the source URL or alt text (`![alt](src)` as literal text inside a code/paragraph node) so the reference survives and the user can act on it. (No bytes are stored — invariant 5 — until the Phase 9 image decision.)
