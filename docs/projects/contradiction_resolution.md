...
> Canonical status is the frontmatter (`done`). Field-discovered 2026-07-14 from a live paid-tier (`gemini-2.5-pro`) session on the "See it in action" demo doc; root-caused the same day. **Design settled 2026-07-16** (§ _Build spec_ below). **Shipped 2026-07-17** (PR pending): `reconcileConflictCardsOnEdit` in `evaluatorReconcile.ts`, threaded from `evaluateSection`. The three open calls were ratified by the owner at build pickup: **(1) close timing — "smart-immediate"** (a refinement of the settled grace-2: close immediately when the edited-side claim is *genuinely gone from the document text* — a whiff-resistant presence check reading the live block text where a verbatim anchor exists, else a forgiving extraction match; only the genuinely *ambiguous* case, wording still present but the pair not re-emitted, waits on B / grace); **(2) B-keep updates the edited side's anchor, message frozen** (as recommended); **(3) B capped at 1/settle and skipped on the weak tier** (as recommended). Verified: full unit/integration suite (9 arm cases + step-4 regression + no-dup-insert) and a live end-to-end drive — a multi-section cross-block Q3-vs-Q2 card resolved by editing the **secondary** (Timeline) section closed as `auto_closed` / `resolved_by_edit`, and a still-conflicting reword was correctly kept.
...
- [x] Owner ratification (at build pickup) of the three settled calls — ratified 2026-07-17: **A-close timing refined to "smart-immediate"** (deterministic document-text presence check → immediate close on genuine removal; grace/B only for the ambiguous still-present-but-not-re-emitted case) · anchor-updates-message-frozen on a B-keep · B capped at 1/settle and skipped on the weak tier.
- [x] Prototype on a branch; drive it in the browser (multi-section cross-block Q3-vs-Q2 doc; resolved on the **secondary** section; card closed `auto_closed` / `resolved_by_edit`; a still-conflicting reword was kept).
- [x] Build `reconcileConflictCardsOnEdit` (new arm in `evaluatorReconcile.ts`): either-side candidate collection, the smart-immediate/re-emit/ambiguous classifier, the gated B confirm, grace bookkeeping — and **route conflict cards out of the span-card decision table's step-4 blanket close** (`existing` now excludes conflict types; the conflict pair-match searches all active conflicts so a secondary-side re-emission coalesces instead of duplicating).
- [x] Thread the two inputs from `evaluateSection`: this settle's `extractedClaims` (already in scope) and the freshly-emitted conflict pair keys (`freshPairKeys`, computed from `newObs`). The B call is an injected `confirmConflict` callback (strong router), keeping the router out of the DB-only reconcile module.
- [x] Regression coverage in `evaluatorReconcile.test.ts` / `evaluator.test.ts`: secondary- and primary-side resolve → close; reworded-but-still-conflicting → kept (B re-anchors, message frozen); reworded-resolved → B closes; weak-tier grace; B cap 1/settle; step-4 no-longer-false-closes a primary-side conflict; no duplicate insert on a secondary-side re-emit.
- [x] Update `docs/mechanics/evaluation-triggers.md` — conflict-card close semantics changed (either-side reconcile via the new arm).
...
2. **A — deterministic classifier**, per card. The *edited side* is whichever anchor sits in `memberSet`; its claim text is `anchorText` (primary) or `conflictingAnchorText` (secondary).
   ...
   - **Gone:** no fresh extracted claim matches the edited side's claim text (match = normalized-text equality, else containment in either direction — the same normalization family `isSpanSuppressed` uses) → the user removed/resolved that claim → close.
     > **Shipped refinement (2026-07-17, owner-ratified "smart-immediate").** The settled spec grace-closed the "gone" case; at build the owner chose to make a genuine removal close **immediately**. The presence check is made whiff-resistant by reading the *live document* first: the edited side's claim is "present" if its verbatim `anchorQuote` is still a normalized substring of the edited block's current text, **or** the stored claim text is literally present, **or** a fresh extracted claim still matches it (equality / containment / `textSimilarity ≥ DOC_DEDUPE_FLOOR`). Only when none of these hold is it "gone" → `auto_closed` / `resolved_by_edit`, no grace wait. Grace (`DOC_GRACE_THRESHOLD = 2`) still applies to the **ambiguous** bucket (wording still present but the pair not re-emitted, and B unavailable — weak tier or the settle's B budget already spent). The reported Q2→Q3 hero case is *lexically similar*, so it lands in the ambiguous bucket and closes via B on the paid tier (immediate); a delete/rewrite-beyond-recognition closes via the deterministic gone path.
