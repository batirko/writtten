...
- [x] Steering contract in the rewritten skill's URL-hosted guidance layer: user focus requests are legitimate and expected; all resulting submissions stay inside the 9-type taxonomy and the register rules; steering narrows attention, never widens the output contract. **Done 2026-07-21** — `docs/skills/writtten-agent.md` § _When the author steers the pass_. **Delivered in the paste, not the URL layer this spec assumed.** The URL-hosted guidance layer was built and then deleted: a real agent session refused the prompt that pointed at it, naming the fetch-more-guidance URL as an injection tell (OBS-040). Rule (e) — move guidance the boundary already teaches behind a URL — does not survive contact with the agent's own trust model, because the cost isn't the guidance, it's what pointing at a URL signals about the sender.
- [x] One machine-validated worked example in `agentSkillExamples.test.ts` — a steered pass yielding an in-taxonomy, register-clean submission. **Done 2026-07-21.** Writing it found a real thing worth recording: the first ❌ drafted for this section — _"Overall the metrics section is solid, though I'd tighten the second paragraph"_ — was asserted to fail `lintRegister` and **passed clean**. A *flattering* rating with a soft prescription slips both the copula-verdict rule (which lists "is weak"/"is poor", not "is solid") and the prescription lexicon (which has "I'd suggest", not "I'd tighten"). The section now teaches two ❌s instead: one the lint genuinely catches, and one labelled as a rating the boundary has no grounds to refuse — the same honesty pattern the lens verdict example uses. Logged as **OBS-041** with the measured evidence — the gap is wider than one example: the verdict rule is polarity-asymmetric (catches "is weak", not "is solid"), and first-person softened prescriptions ("I'd tighten") are absent from the lexicon that has "I'd suggest".
...
~~Per the rework's own design rule (e), lens guidance belongs in the URL-hosted layer.~~ **Superseded 2026-07-21:** there is no URL-hosted layer. It was built, the prompt that pointed at it was refused by a real agent session as prompt injection, and it was deleted (OBS-040 · `agent_connected_eval.md` § The skill). Lens guidance ships **in the paste**, alongside everything else. Rule (e)'s reasoning was sound about *redundancy* and wrong about *cost*: guidance the boundary re-teaches on rejection is indeed redundant, but removing it is not free — the volume of framing is part of what makes the paste legible as a document rather than a payload. Calibration's exception still stands and is now one of several reasons the paste stays whole.
