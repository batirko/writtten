...
**Revised on sight, 2026-07-22 (owner).** Two things off in the shipped tooltip. (1) It rendered as a dark slab with only the "What this is" link visible — the two sentences of explanation were a bare text node painted *behind* the card's `::before` background sheet, while the raised `<a>` alone showed through; a plainly bad look and, worse, uninformative. Rebuilt as a **light info-card** (`.engine-tip-card`: surface bg, hairline border, soft shadow) with the content in a real element, so nothing hides. (2) The copy said only **"Safari"**, but the block is WebKit-on-https, which also catches the **in-app browser some AI coding tools embed** — and that is precisely where a BYOA user (running Claude Code / Codex / Cursor and pointing its browser at writtten) is likely standing. New copy: *"Safari and some in-app browsers — including those in AI coding tools — can't reach a bridge on this machine. Open writtten in Chrome, Edge, or Firefox to connect an agent."* Named as a **category, not per-app**: a specific tool's embedded engine can change under us, and the CLAUDE.md warning against per-browser claims that rot applies to per-app ones too. **Follow-up owed (owner, 2026-07-22): carry the same in-app-browser framing into `public/agent/index.html`** — its Safari section still says only "Safari", and a user who lands there from an AI-app browser should recognise their own situation.
