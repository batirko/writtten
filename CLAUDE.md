...
> **Index links are load-bearing — don't let a WYSIWYG editor eat them.** Each index row's name must stay an inline link `[name](projects/name.md)`; the test rejects bare text. Root cause of the repeated de-linking (it shipped once — see the L1 milestone below): the "Markdown Studio" VS Code extension registers a WYSIWYG custom editor for `*.md` (priority `default`, autosave on) and re-serializes on save, dropping links inside table cells. The repo's `.vscode/settings.json` pins `workbench.editorAssociations` so `*.md` opens in the plain-text editor and is never re-serialized. If the index ever gets de-linked anyway, `npx prettier --write docs/plan.md` restores nothing (the links are already gone) — restore from git (`git checkout -- docs/plan.md`) or re-add the links, then run `npx prettier --write docs/plan.md` only to re-align the table. CI (`npm test`) is the backstop.
