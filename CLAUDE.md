...
> **Index links are load-bearing — don't let a WYSIWYG editor eat them.** Each index row's name must stay an inline link `[name](projects/name.md)`; the test rejects bare text. Root cause of the repeated de-linking (it shipped once — see the L1 milestone below): the "Markdown Studio" VS Code extension opens `*.md` in a WYSIWYG editor and round-trips md → html → md on save, dropping links inside GFM table cells. Two guards in `.vscode/settings.json`: `markdownStudio.autoSave: false` (opening a file no longer silently rewrites it) and `workbench.editorAssociations` forcing `docs/plan.md` into the plain-text editor (never the WYSIWYG one). **Editing _and saving_ any link-in-table file in Markdown Studio still flattens its links** — edit such files in the plain-text editor. If the index gets de-linked anyway, `git checkout -- docs/plan.md` to restore (Prettier can't recover lost links; it only re-aligns the table once links are back). CI (`npm test`) is the backstop.
