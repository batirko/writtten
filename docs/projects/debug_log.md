...
> **Readiness pass 2026-07-16 — the three open bullets specced to build-ready.** Two facts have moved under this slice since it was written: (1) a **partial unification already exists** — `llmLogger.setEventSyncHook` pushes logger records into the harness (wired DEV-only since L7), so "unify" is no longer a greenfield emitter but *finishing a one-directional sync into a two-vocabulary merge*; (2) the router went **multi-provider** (2026-07-07), so "capture Gemini `usageMetadata`" is now an adapter-seam task, not a Gemini patch.

- [ ] **Unify — one buffer, two views.** Keep `llmLogger` as the single owner of the ring buffer; extend its D1 discriminated union with the harness-only record kinds (`settle`, `ledger-write`, `observation`, `block-removed`) and have `harness.emit()` delegate into it (inverting today's one-way logger→harness hook into "harness writes through the logger"). `getEvents(sinceSeq)` and the debug panel become two **projections** over that one buffer: the agent view keeps its monotonic `seq` + oldest-first contract (assign `seq` at the unified write point so existing acceptance tests hold); the panel view keeps newest-first rendering. Retire the duplicate buffer in `harness.ts` and the `setEventSyncHook` seam. One retention limit (500, superseding 50-vs-500) — see the retention bullet. **Guards:** the existing `getEvents` consumers (`acceptance.phase1.test.ts`, waiting-for-idle loops) and `logger.persistence.test.ts` (the sessionStorage mirror now mirrors the unified buffer) must pass unchanged — they are the contract that the merge broke nothing.
- [ ] **Token capture at the adapter seam; cost stays optional.** Each `ProviderAdapter` maps its native usage shape — Gemini `usageMetadata.{promptTokenCount,candidatesTokenCount}`, OpenAI `usage.{prompt_tokens,completion_tokens}`, Anthropic `usage.{input_tokens,output_tokens}` — into a normalized `tokens?: { input: number; output: number }` on the parsed response, stored per attempt on the `CallRecord` and surfaced in the panel + envelope beside `latencyMs`. **Per-call dollar cost is deliberately second-class:** it needs a per-model price table that rots and mispricing is worse than not pricing (free-tier calls cost $0 anyway) — ship tokens first; if cost lands at all it's a static `PRICE_TABLE` with an `asOf` date rendered as "≈", DEV-panel-only.
- [x] Redaction pass on the export envelope (ensure API keys never leave the `<free>`/`<paid>` masking; confirm no key material in `endpoint`). **Shipped 2026-07-12 (#179).** Confirmed the defence-in-depth: the endpoint URL's key is masked to `<free>`/`<paid>` at log time (`rotation.ts`) and the `endpoint` field is dropped from the envelope entirely (`buildEnvelope`). Closed two residual free-text vectors — `redactKeys` now also masks bare `AIza…` / `sk-…` keys (not just the `key=` form), and it is applied to the `user` and `response` fields (a user who pastes a key into the draft body no longer leaks it). Regression test in `debugLog.test.ts` seeds a raw key into endpoint/system/user/error/response and asserts the serialized envelope contains no raw key (+ a no-false-redaction prose case).
- [ ] **Retention review — measured, not guessed.** Method: paste one of the large V1 corpus PRDs in mock mode, run to idle, and read the unified buffer's record count + serialized envelope size (one `javascript_tool` eval). Acceptance: the buffer holds **a whole bulk-paste session plus the follow-up doc pass** without evicting the paste's own triggers (today's `maxLogs = 50` demonstrably cannot — a 20-section paste alone exceeds it); the sessionStorage mirror stays comfortably under quota (~1–2 MB envelope budget). Expected landing zone ≈ 500 records / start eviction oldest-first by record (never by kind); tune from the measurement, and record the numbers here.
