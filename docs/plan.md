...
- [ ] **Canvas content types (paste-in & create)** — decide and bound which rich-content types the canvas accepts on paste and lets the user create (tables, links, images, task lists), and how each interacts with the section/eval model + Markdown round-trip. **Phase 6 floor: stop silent content loss** — today StarterKit-only means a pasted table/image vanishes; at minimum degrade unsupported paste to a visible/editable form. Tables are the highest-value PM-persona add. Distinct from _Editor formatting UX_ (that's the control surface; this is the schema). → see `docs/projects/canvas_content_types.md` — 🟠 Med · 🧠
- [ ] **Impact legibility v2 (R7a follow-up)** — the shipped severity/confidence cue (a 6px `impact-cue` dot + `title` tooltip + the kind×severity left-border) is **too subtle as a first layer**, and the native-`title` tooltip is unreliable (cursor shows `help` but no tooltip appears — `SidecarFeed.tsx:114`/`styles.css:322`). Make the first-glance impact obvious (legible cue, not a near-invisible dot) and replace the native `title` with a real tooltip (or at least verify/repair it). Owned jointly with Visual style (badge language) and emotional_register (what the cue says). → see `docs/projects/quality_remediation_synthesis.md` (R7a) · `docs/projects/visual_style.md` — 🟡 Med · 🧠
...
- [ ] **V1 — Base-rate corpus study.** Run the pipeline over 15–20 real PRDs; count un-planted true contradictions (hero base rate), per-type precision in the wild, and the free-vs-paid-tier delta. **Pulled forward from the Phase 7 taxonomy research gate.** — 🟡 Med · 🧠
