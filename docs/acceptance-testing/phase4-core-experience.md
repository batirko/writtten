...
>
> - **Browser pane** (built-in `mcp__Claude_Browser__*`, loaded by default) — preferred for anything driving `window.__sidecar__`: seeding fixtures, polling state, reading the ledger/observations, record/replay. Drive it with `javascript_tool` (pass a single async IIFE expression — no top-level `await`/`return`); `read_page` for the a11y tree, `computer` for input/screenshots. Per-session and collision-proof.
> - **chrome-devtools** (`mcp__chrome-devtools__*`) — fallback for real input fidelity: typing into the ProseMirror editor, hovering cards to trigger highlights, clicking by accessibility uid, reading rendered CSS (border colors, the `~` qualifier). Shared global selection — see the ⚠️ callout in `CLAUDE.md`.
>
