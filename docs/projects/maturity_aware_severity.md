...
- [ ] **Maturity gates doc-level readiness, not a raw word count (UX-013).** Extend the proxy to also answer "is this draft complete _enough_ to earn the doc-level 'how it fits overall' pass" — so a **structurally-complete short draft** gets gentle fit observations rather than being silenced by the hard `CONTENT_THRESHOLD_WORDS` (150) `doc-idle` cliff (the 2026-07-02 session: a finished short essay never received any doc-level fit). Replace/augment the raw word-gate arming of `doc-idle` with the maturity proxy; keep the quiet-while-forming posture (a genuinely half-formed draft still stays silent). → `docs/logs/ux_quality_observations.md` (UX-013).
...
- [ ] **Compose with R3c.** A mid-session promotion is a _changed_ observation → it correctly fires the existing R3c "New/changed" marker ("this got more serious as your doc matured"). Reuse; don't suppress.
...
- [ ] **Calibrate + validate.** Thresholds and voice ship **provisional**; V1 calibrates the maturity cut-offs against real PRDs, V2 tests the OBS-010 hypothesis (does located critique on a _mature_ draft land as respect, not coldness — and does softening on an _early_ draft remove the abrasiveness). Update the open question in `docs/plan.md` → Strategic open questions when V2 answers it.
