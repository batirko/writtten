...
- [ ] **Rhetoric not extracted as `fact_claim`** — under relaxed document classes, stop promoting hyperbolic/rhetorical emphasis and narrative framing to ledger `fact_claim`s; this is the upstream source of the false `unsupported_claim` + `strategic_tension` cascade on essays. Composes with `document_type_calibration.md` (extraction-side calibration). (See OBS-037.)
