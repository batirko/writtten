...
- [x] **Document-scope legibility** — doc-level observations (no span) now show a card-intrinsic **"Whole doc"** scope-marker chip in the quote slot and **shed** the hover/click locate affordance (rather than gaining an editor-side hover indication) (§ C6). Built & browser-verified 2026-07-09. → see `docs/projects/doc_scope_legibility.md`
...
- [x] **C8 — Span→card reachability / pin-on-click** — **shipped.** Clicking a highlighted span pins its `SpanPeek` card float (`spanPinnedObsId`); while pinned the reverse-hover dwell is suppressed so the float stays put (the "N more" drawer becomes reachable). Dismiss on Escape / click-away / × (new × control on the pinned peek). Caret-safety: pins only on a click with an empty selection and no drag, reusing C9's covering-set resolution; caret placement / text selection untouched. Tap-to-pin is the touch affordance (§ C8).
- [x] **C9 — Overlapping / co-located-highlight targeting** — **shipped.** Reverse hover now resolves the covering _set_ by coordinate (`posAtCoords` → `computeObservationRanges` hit-test in `Editor.tsx`, reusing the highlighter's offset→pos mapping), surfaces **all** covering cards, and picks the **smallest range as primary** so a nested substring is reachable (symptom i) (§ C9). A downgraded invisible-anchor substring is targetable inside a visible span; plain text covered only by invisible anchors stays inert (surfaced-only cue). `resolveCoveringSet`/`computeObservationRanges` are unit-tested; cross-claim dual-span preserved. **Rendering (revised):** the whole covering set (`spanFocusRelatedIds` → `spanPeekGroups`) **stacks in the `SpanPeek` float** (most-specific first) over a uniformly dimmed feed — the first cut un-dimmed the co-covering cards _in the feed_ instead, which collided awkwardly with the float that assumes everything behind it is dimmed (field-reported). One coherent stacked float, no loose feed cards; also fixes the collapsed-feed case where in-feed un-dimming did nothing.
- [x] **Mid-sentence quote: ellipsis prefix, no forced capital (UX-008 refinement)** — shipped 2026-07-08. The `.card-anchor` quote now formats through `formatAnchorExcerpt` (`src/sidecar/anchorExcerpt.ts`): a verbatim excerpt leads with `…` when it starts mid-sentence, keeps its original casing, and trails with `…` when cut before a boundary (§ UX-008 note).
- [x] **C10 — Drop the redundant canvas focus outline** — removed the full-canvas `.tiptap:focus-visible` ring; the caret is the focus signal. Global `:focus-visible` + discrete-control rings stay intact (§ C10).
...
#### C6 — Document-scope legibility (BUILT — 2026-07-09, revises the original hover-affordance framing)

Doc-level observations (`missing_topic`, `structure_flow`, `audience_mismatch`, doc-scope `underexposed_topic`) have no span. The original framing proposed an editor-side hover affordance (a calm edge indication on the column). **What shipped instead (`docs/projects/doc_scope_legibility.md`) is a card-intrinsic marker plus honest affordance removal** — the more faithful resolution, since there is genuinely nothing on the canvas to point at:

- The card's context-line slot shows a **scope marker** (`.card-scope` chip: a document glyph + **"Whole doc"**, muted sans) where a span card shows its serif-italic quote. Keyed on `scope === "document"` (`isDocScope` in `GroupedObsCard`), so dual-scope `underexposed_topic` is covered by data, not a type list.
- The **locate affordance is removed**, not replaced: doc cards don't wire `onHover` (no span to highlight), the card body no longer dispatches `obs-card-activate` on click/`Enter`, and `.observation-card-docscope:hover` drops the hover-lift. The card reads as static. Dismiss + the "N more" toggle keep their own behaviour.

No full-canvas flash, no editor-side edge indication — the signal lives entirely in the feed card.
...
**One root cause, two symptoms.** Reverse hover resolves the dwelled span with `closest(".obs-highlight[data-obs-id]")` in `Editor.tsx` — the **pre-C9** mechanism, since replaced by the coordinate hit-test described above — which walks up to the **nearest ancestor highlight element** and reads **exactly one** `data-obs-id`. So whenever more than one observation decorates the same text, only one card can prime:
...
- **(ii) Co-located (multiple cards, same span).** _Several_ observations cover the **same** span, so the feed shows several cards for it — most visibly a claim that participates in **multiple contradictions** (the 2026-07-07 screenshot: "Push notification arrives within 10 seconds…" is one side of three separate contradiction cards, each its own group because grouping keys on `blockId:startOffset:endOffset` and each contradiction's other side differs). Hovering the shared span primes only **one** of the three cards. (Note: truly identical-span observations _do_ aggregate into one group — `groupObservations` in `obsAggregation.ts` — so this symptom is specifically observations whose spans _coincide but aren't identical_, e.g. the same text is the `blockId` side of one contradiction and the `conflictingBlockId` side of another.)

**Root-cause hypotheses (confirm at build):**

- **(a) The substring observation is downgraded** ("also noticed", outside the feed budget) → it renders as an **invisible anchor** with no `obs-highlight` class (`showMark = surfaced || isHovered || isPulsing`, in `ObservationHighlighter.ts`). So `closest(".obs-highlight[data-obs-id]")` **skips it** and lands on the visible outer span. This fits "always only the bigger highlight was primed" if the bigger one is the surfaced/visible one.
...
Scope note: this is **no longer a pure card-render change** — it adds an observation field + an emit-time excerpt derivation (small, offset-slice; no prompt/re-record needed since it reads offsets we already compute). Couples with OBS-032.

**Backend shipped 2026-07-07 (emit only; render is the Feed session's).** `anchorClaimsToMembers` (`evaluatorAnchoring.ts`) now stores `anchorQuote` on a **precisely** anchored claim — the exact source slice `member.text.slice(start, end)`, the user's verbatim (possibly mid-sentence, lowercase) words — and leaves it undefined on the OBS-032 whole-body-block paraphrase fallback (no faithful excerpt). It's carried on `ClaimLedgerEntry` and emitted onto the `Observation` at both cross-claim sites (`evaluator.ts`: sweep `anchorQuote: a.anchorQuote`; per-section new side sliced from the emit's own `exact` match). `anchorText` is untouched (still the normalized claim used for matching/suppression). **No DB migration** — the field is additive/optional and the render falls back to `anchorText`, so old observations simply lack it and refresh on re-eval (matches the `ClaimLedgerEntry` anchor-field precedent). Unit-tested in `evaluatorAnchoring.test.ts` (verbatim slice, mid-sentence lowercase excerpt, absent on fallback) and `evaluator.test.ts` (sweep emits `anchorQuote` from an anchored claim; undefined for whole-block).

**Render shipped 2026-07-08 (Feed session).** The card quotes `anchorQuote ?? anchorText` through the pure helper `formatAnchorExcerpt` (`src/sidecar/anchorExcerpt.ts`). An excerpt is treated as **verbatim** — and so gets the ellipsis fencing — when it's a span check (its `anchorText` is the flagged `obs.substring`) or a cross-claim card that captured the precise slice (`anchorQuote != null`); the cross-claim **paraphrase fallback** (normalized capitalized claim, no `anchorQuote`) renders plainly as a standalone sentence. For a verbatim excerpt: lead with `…` when it starts with a lowercase letter (mid-sentence), keep its casing (no forced capital), and trail with `…` when the last char isn't sentence-terminating punctuation (optionally followed by a closing quote/bracket). The CSS one-line `text-overflow: ellipsis` still handles visual overflow; the `title` tooltip carries the raw excerpt without the formatting ellipses. Unit-tested in `src/sidecar/anchorExcerpt.test.ts`; the SidecarFeed span-anchor render test was updated to the fenced form; verified live against the zero-config example (`…nearly a third…`, `The public launch is firmly set for Q3 2026…`).

#### C10 — Drop the redundant canvas focus outline

**The problem.** The keyboard-first / tabbable a11y pass added `.tiptap:focus-visible { outline: 2px solid #0ea5e9; outline-offset: 8px }` in `styles.css`, which paints a sky-blue ring around the **entire writing canvas** whenever it's focused. For a `contenteditable`, keyboard interaction makes it `:focus-visible` essentially **the whole time the user is writing**, so the ring is almost always on. It's **redundant** (the text caret already signals focus) and reads as chrome against the calm editorial canvas (`visual_style`).
...
- A new card animates in with the standard reveal — fade + small `translateY` — using `--dur-base`/`--ease-out`, `transform`/`opacity` only. The existing batched-arrival path (`arrivalBatchCount` in `SidecarFeed.tsx`, "+N new" when ≥3 land at once) is **kept and generalised**: single/few arrivals get the same per-card enter, not just batches of 3+.
...
- **Source (settled):** the **stored `anchorText` snapshot** already on the Observation (`db.ts`) — the same field the archive ghost-anchor uses. No live span re-resolution; consistent with the archive, zero new resolution logic. It can go mildly stale between evals, but reconciliation supersedes/refreshes the card when the span changes, so the snapshot tracks closely enough.
