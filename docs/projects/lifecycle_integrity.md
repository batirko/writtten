...
- [ ] **5c — Unify per-section conflict identity on `conflictPairKey`.** `reconcileObservations` should dedupe/supersede `contradiction`/`strategic_tension` by `conflictPairKey` (not `contentSig`/`spanSig`) so a per-section conflict and the sweep's re-emission of the same pair coalesce into one card. (The *suppression* side of this is already done in 5a; this is the reconcile/dedupe side.) — 🟡 Med · 🧠
- [ ] **5b — Re-anchor highlights by `anchorText` on rebuild.** `ObservationHighlighter.ts` correctly `map()`s decorations between meta updates, but `refreshObservations` (`App.tsx`) fires on every eval and rebuilds **all** decorations from `obs.startOffset/endOffset` as stored at eval time — so a highlight visibly jumps if the user typed earlier in the block. Re-anchor by `anchorText` (offset fallback; never re-anchor `0:9999` sentinels) on rebuild. — 🟡 Med · 🧠
