...
The copy names a **category, not "Safari"**: *"Safari and some in-app browsers — including those in AI coding tools — can't reach a bridge on this machine. Open writtten in Chrome, Edge, or Firefox to connect an agent."* The block is WebKit-on-https, which also catches the in-app browser some AI coding tools embed — exactly where a BYOA user (running Claude Code / Codex / Cursor) is likely standing — and a per-app capability claim would rot, so the category is deliberate. (A matching broadening of `public/agent/index.html` is an owed follow-up, 2026-07-22.)
