...
**Date:** 2026-07-21\
**Status:** open — **direction settled by the owner the same day** (see the Phase 8 milestone). Confirmed by the owner in Safari and by reading; the slot-capture half is code-confirmed, not yet observed live.\
...
**Notes:** `agentBrowserSupport.ts` is already the right predicate and already pure and argument-injected, so this is wiring an existing check into three call sites, not new detection. Note its deliberate scope — WebKit **and** `https:` only, so an `http://localhost` self-hoster keeps a path that genuinely works; the fix must inherit that nuance rather than sniffing for Safari afresh. iOS Chrome and Firefox are WebKit underneath and are correctly caught by the `navigator.vendor` predicate.
