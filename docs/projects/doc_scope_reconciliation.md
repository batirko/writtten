...
> The block-level reconciliation engine was repaired in Phase 4 (R3 — span+text matching, prior-observation re-injection, `resolved_prior`). The **document-scope** path was not. It still does type-bucketed _positional_ supersession, which manufactures false "superseded" links, flickers stable notes, and silently drops still-true critiques. This file captures the finding, an implementation plan for the high-value / low-ambiguity correctness fix, and the open decisions for the larger reassessment work.
...
- `docs/projects/observation_taxonomy_and_priority.md` — the budget feed (`DEFAULT_FEED_BUDGET = 7`) that determines whether these doc-scope notes were ever _visible_ when they churned.
...
3. **`resolved_prior`** — the model _explicitly_ said the observation no longer applies.
...
It grabs the **first unmatched existing observation of the same type**, regardless of topic. Because assignment is greedy and order-dependent, the 0.6 dedupe only fires if the matching slot is _still free_ when its new counterpart is processed — an earlier, unrelated new observation can consume it first.
...
Recommended "right thing." The price is **not** extra API requests — the binding free-tier constraint is **requests-per-day per model**, and this folds the resolution question into the _existing_ single doc-quality call (the section-eval already proves this pattern at ~1s on the fast tier).
...
### Decision (UX-012, settled 2026-06-26): the hybrid

Neither single direction is sufficient: routing _everything_ through the reconciler still wastes a re-grade on a none→suggested transition where nothing changed, and skipping _only_ the wipe leaves genuine re-scopes on the broken blind-supersede path. So **both**, keyed on the nature of the transition:

1. **Auto-applied none→suggested → skip the wipe entirely.** When the stage change is the user accepting the model's _own_ inferred `suggested_stage` from a `none` starting stage (not a hand-edit), the document content is unchanged — the only thing that moved is the stage label. Do **not** supersede or regenerate; leave the existing doc-scope observations in place and let the next ordinary `doc-idle` reconcile them resolution-aware. This is the worst-case path from the 2026-06-25 session and the cheapest to neutralise.
2. **Genuine (hand-edited) stage change → route through the resolution-aware doc reconciler.** When the user deliberately re-scopes (e.g. PRD → email, or edits the stage field by hand), re-grading is correct — but it must go through the **same** resolution-aware path `doc-idle` uses (Workstream A: priors injected with indices, keep-by-id on persists, lexical best-match + `DOC_GRACE_THRESHOLD` grace as fallback), **not** the blanket `superseded` + blind regen. So a re-scope that leaves most critiques still true keeps them by id instead of churning the archive.
...
This is build-ready and tracked as the Phase-6 Signal-quality **Stage-change reconciliation (UX-012)** milestone; it extends the already-`done` Workstream-A machinery rather than adding new dependencies.
...
- Does the grace period interact badly with genuine user edits that _do_ resolve a doc-scope note (delayed "✓ addressed" by N runs)? D2 resolution-awareness would close a true-resolution faster than hysteresis-only.
